...
<ResponseField name="batches[i].base_prompt" type="string">
  ...
  original placeholder variables such as `{{ business }}` or `{{ name }}`.
</ResponseField>
...
<ResponseField name="batches[i].endpoint_code" type="string" default="API">
  Enterprise customers with custom endpoints will see the endpoint code here if
  specified.
</ResponseField>

<ResponseField name="batches[i].call_params" type="array">
  The base call parameters used to create the batch, such as `voice_id`,
  `max_duration`, `reduce_latency`, and `wait_for_greeting`.
</ResponseField>
