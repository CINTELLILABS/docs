...
<ParamField body="amd" type="boolean" default="false">
  ...
  HOWEVER: AMD mode also causes increases latency. It will cause worse performance when speaking with normal people. You should default to switching it off (`false`)
</ParamField>
...
<ParamField body="dynamic_data" type="array">
  An array of objects that allows the AI to fetch and use live data from external APIs during the call. This dynamic data can be used to provide up-to-date information or context-specific details in the conversation.

  Each object in the array can specify a URL to fetch data from, the method to use (e.g., GET, POST), whether to cache the data, and how to parse and use the response data.
...
"dynamic_data": [
  {
    ...
    "response_data": [
      {
        "name": "DataName",
        "data": "json.path.to.data",
        "context": "Use this data like this: {{DataName}}"
      }
    ]
  }
]
...
<ParamField body="dynamic_data[i].url" type="string">
The URL of the external API to fetch data from.
</ParamField>

<ParamField body="dynamic_data[i].method" default="GET" type="string">
The HTTP method to use for the API call. Allows `"GET"` and `"POST"`.
</ParamField>

<ParamField body="dynamic_data[i].cache" default="true" type="boolean">
Whether to fetch the data once at the beginning of the call, or to re-check continuously for data that might change mid-call.
</ParamField>
