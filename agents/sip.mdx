...
Open **TLS 5061** to the signaling IPs and **UDP 16384 to 32768** to the media IPs for your region. You can also copy the list from the **Firewall & Ports** panel on the trunk page, or fetch it with [`GET /v1/sip/firewall-ips`](/api-v1/get/sip-firewall-ips).

<Note>
  Whitelisting is mutual: Bland allow-lists your source ranges, and you allow-list Bland's signaling and media IPs. Open RTP (UDP 16384 to 32768) to the media IPs in addition to TLS 5061 to the signaling IPs, or audio will not flow.
</Note>
