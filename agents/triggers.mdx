...
Each card also reports its own state. An Action card reads **Not configured** under the action name until every required field is filled.

The **Ready** and **Setup required** badges are slower: they appear only once you have opened that step and moved off it, so on a first pass through the builder there is nothing on the cards to read.
...
<Steps>
  <Step title="Pick the event">
    ...
    A non-Bland event then asks which connection to use. If you have none, **Connect** opens a panel whose only button takes you to **Tools** in a new tab, which is where connections are actually made. Create it there, come back, and reopen the step.
  </Step>
  ...
  <Step title="Choose the action">
    Five kinds: **Call**, **SMS**, **Webhook**, **Slack message**, and **Custom code**. The list shows only the ones your event supports, and if you change the event to one that does not support the action you had picked, it quietly reselects the first that fits.

    Every CRM event offers **Call** and **SMS** and nothing else, because the record it carries is a person to reach. Bland's own events vary, and seven of the fourteen offer less than all five:
    ...
    On a **Call**, the agent picker sits above the action type, because it is the first thing to decide. Below it, **Send from** picks the number the call goes out on, from the numbers in [Dispatch](/agents/dispatch), and **Send to** is the number it dials. Picking the agent fills **Send from** in with that agent's own number. If you own no numbers at all the field is replaced by a note saying calls go out from a shared Bland number, which is not an error.

    A **Slack message** picks its own workspace and channel, whichever event started it.
  </Step>
  ...
  <Step title="Point the action at the event">
    ...
    **Send to**, and each **Request data** row, carries a **Static** / **Event** switch on a **Call** and on an **SMS**. **Static** is a value you type, the same on every run. **Event** reads it off the payload that fired the trigger. So **Send to** can be the phone number on the case rather than one number you typed once.

    **Request data** is key and value pairs handed to the agent as context. The agent sees both the keys and the values, so name them the way you would say them.

    A **Webhook**'s URL and its headers carry the same switch with a third option, **Org Default**, which uses the webhook URL from your organization settings.

    The other two actions reach the event a different way. An **SMS** or **Slack** message body has no switch: use **Insert field** above the box, which writes a `{{field.path}}` token into the text. **Custom code** gets the whole payload as `input`.
  </Step>
...
The button is not disabled and no message appears. What you get is the field label turning red, in whichever step you are looking at, and the Action card on the canvas reading **Not configured**.

The **Setup required** badge is the signal you would want and the one you cannot count on, because it waits until you have opened that step and left it. So when Save does nothing and nothing on screen is flagged, click each card on the canvas in turn. That takes you to the step and marks it visited, which is what makes the badges start working.
</Warning>
...
That has a consequence worth knowing before you build one: only a **Call** trigger is ever filed under an agent. A webhook, SMS, Slack or custom code trigger lives at **All agents** and nowhere else, even when you build it from inside an agent.
...
<AccordionGroup>
  ...
  <Accordion title="The trigger is not in the agent's list">
    If it places calls, save it a second time. Save, then save again from the page it lands on, and it appears in both places: the first save writes the trigger and its call settings, and the agent it is filed under is written by the update that follows.
