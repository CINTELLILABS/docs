...
| Tab | What it covers |
|---|---|
| **Calls** | How the agent answers the phone: maximum call length, recording, transcription keywords, and what to do with voicemail. |
...
<Warning>
Voice settings apply to every number the agent has. Messaging settings, and a number's language, are saved separately for each number.
</Warning>

### Transcription keywords

Transcription keywords are words and phrases the transcriber should expect, like product names, brand names, or unusual surnames. Add them when the agent keeps mishearing a term callers say often.

They are on the Calls tab, in the **Call behavior** card next to **Call recording**. Type a word or phrase and press Enter to add it. Select the tag's remove button to take one out.

- Up to 20 keywords, each up to 100 characters
- To boost a keyword, add a colon and a whole number, like `Bland:2`. That is the only place a colon is allowed

Keywords are an agent setting. They apply to inbound calls on every number the agent has, and to outbound calls the agent places. Keywords you pass on an individual call replace the agent's list.

### Set a number's language

A number's language sets the speech recognition language for inbound calls to that number. It also selects the voice model and prompt language.

<Steps>
  <Step title="Open the number's advanced settings">
    On the agent's Channels page, open the **Number options** menu on the number's row and select **Advanced**.
  </Step>

  <Step title="Pick a language">
    Under **Language**, below **Timezone**, search for and select a language. Select **Clear** to unset it.
  </Step>

  <Step title="Save">
    Press **Save** in the drawer. The drawer saves on its own, separately from the builder.
  </Step>
</Steps>

For an inbound call, Bland uses the first language it finds in this order:

1. The number's language, if you set one
2. The agent's language, from **Languages** on the builder's Settings tab
3. English

Outbound calls have no number language to read. They use the language passed on the call, then the agent's language, then English.

<Note>
With no language set anywhere, the call recognizes English only. To recognize any language, set the number's language to **Babel (All languages)**.
</Note>
