...
<Steps>
  ...
  <Step title="Attach it to the agent">
    Press Configure on the number's row. In the sheet, find the line that says who owns the number and use the dropdown beside it. It has no label of its own. The line reads Not assigned to an agent for a free number, or Answers pathway followed by the pathway's name for a number that still answers a pathway from the previous dashboard.
    ...
    A number that still answers a pathway gets a dialog headed Take this number off its pathway? instead, naming the pathway it leaves. **Take off pathway and assign** goes through. This dashboard cannot put the number back on its pathway, so confirm only when the agent is ready for its calls.
  </Step>
...
You know it worked when the number's Assignment column shows the agent's name, and the agent's Channels page lists the number under Numbers. Before you assign it, a number that still answers a pathway shows Pathway: followed by the pathway's name in that column, with Previous dashboard beneath it.
