...
The number follows the agent. Rename the agent and the Assignment column shows the new name straight away, with no need to attach the number again. Delete the agent and its numbers are released. They show as belonging to nobody, and calls to them no longer reach the deleted agent. Attach them to another agent to put them back into service.
