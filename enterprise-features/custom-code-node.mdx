...
Custom code can call out to your connected integrations and reference stored secrets without managing credentials yourself. Two extra keys are available on the request payload:

- `input.connections` — your authenticated integration resources (Salesforce, HubSpot, Slack, custom APIs, and so on), keyed by an alias you choose.
- `input.secrets` — your stored secrets, keyed by an alias you choose.

Connections are configured in the [Tools Hub](https://app.bland.ai/dashboard/tools) under **Connections**, and secrets are managed in the [Secrets](https://app.bland.ai/dashboard/secrets) dashboard. Once a connection or secret exists, you can wire it into a custom code node by alias.

Reference a secret using the `{{SECRET.<name>}}` syntax — Bland resolves it at runtime so the value never appears in your code.
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Use a connected integration resource
    const salesforce = input.connections.salesforce;

    // Read a resolved secret
    const apiKey = input.secrets.api_key;

    const response = await fetch("https://api.example.com/orders", {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "X-Salesforce-Resource": salesforce.resource_id,
      },
    });
...
Aliases (`salesforce`, `api_key` in the example above) are the keys you choose when configuring the node. Use whatever is meaningful for your code.
