...
### Execution timeout
Custom Code Nodes that run before the agent speaks are bounded by a 10 second wall-clock timeout by default. If a snippet does not return within that window, the node fails, the agent proceeds without the snippet's returned variables, and the caller no longer sits in silence waiting for the code to finish.

You can override the default per node in the **Execution Timeout** field of the Custom Code node drawer, which accepts a value in milliseconds between 1000 and 60000 (1 to 60 seconds). Leave the field blank to use the default. Use a lower value to fail even faster when a snippet must not block the greeting. Use a higher value when the snippet legitimately needs longer to complete a call to an external service before dialogue starts.

If the code returns before the timeout, the snippet's variables are available in later nodes as normal. If it does not, the node takes the existing failure path and the pathway continues to the next node.
