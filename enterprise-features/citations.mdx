...
Citations allow you to define structured post-call and post-conversation analytics for enterprise use cases. By configuring schemas of variables and logic, you can automatically extract key information from call transcripts and SMS conversations — no manual review required.
...
Citations can be included in your outbound or inbound calls using the `citation_schema_ids` parameter in the [Send Call](/api-v1/post/calls#param-citation-schema-ids) or [Update Inbound Number](/api-v1/post/inbound-number-update#param-citation-schema-ids) API endpoints. For SMS conversations, pass `citation_schema_ids` in the [Send SMS](/api-v1/post/sms-send) or [Send SMS Batch](/api-v1/post/sms-batch) endpoints, or configure them on the SMS number directly.
...
For transferred calls, you can opt in to receiving the conversation after the handoff on the citations webhook. Set [`include_post_transfer_transcript: true`](/api-v1/post/calls#param-include-post-transfer-transcript) on the outbound call (or on the inbound number). When the call transfers to a human, the citations webhook payload gains:

- `post_transfer_transcript` — the corrected transcript for the portion of the call after the transfer, with the transferred-to human labeled `representative` and the caller labeled `user`.
- `transfer_offset_seconds` — the number of seconds from the start of the recording at which the transfer occurred.

The existing `corrected_transcript` field still ends at the transfer boundary, so citation extraction, evals, and enrichment continue to score only the AI portion of the call. Recording is required — requesting the post-transfer transcript without `record` enables recording automatically, and setting `record: false` alongside it returns an error.

If you need the post-transfer conversation as it happens — for agent-assist prompts, live monitoring, or supervisor dashboards — use the [live post-transfer transcript stream](/api-v1/post/calls-id-transcript-stream-token) to open a WebSocket that emits finalized segments in the same shape as the webhook payload.
---
