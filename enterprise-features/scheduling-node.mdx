...
The Scheduling Node supports the same deterministic response-based routing as the [Webhook Node](/tutorials/webhooks). After every request to your Available Slots API, Bland can send the caller to a different pathway node based on the HTTP status code or a value extracted from the response body — before any booking conversation begins.
...
Rules are evaluated in order from top to bottom, and the last matching rule wins — the same behavior as Webhook Node routing. If no rule matches, the Scheduling Node continues exactly as before: the agent offers the returned slots and confirms the appointment with the caller.
