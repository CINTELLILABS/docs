...
* **Transferring calls** to DIDs, extensions, and SIP URIs on your trunk
...
Trunk health is shown as one of three badges on the trunk header:

- **Healthy** — the endpoint replied to the SIP OPTIONS ping.
- **Mutual TLS** *(amber)* — the endpoint completed the TLS handshake but requested a client certificate instead of replying to the OPTIONS ping. Calls are still expected to work; this is common for carrier SBCs that gate signaling with mutual TLS.
- **Unreachable** *(red)* — no response over TLS.

You can also delete a trunk or detach individual numbers directly from the dashboard. Both actions open a confirmation modal that names the trunk (and, for deletion, the number of attached numbers that will stop routing).
...
When your DNS `_sips._tcp` SRV record resolves the endpoint, the port is left unpinned and shown as **Auto (SRV)**. This lets Bland re-resolve SRV per call for load balancing and failover — pin an explicit port only if you need to override that. If the endpoint answers TLS but requests a client certificate instead of replying to the SIP OPTIONS ping, discovery reports it as **Endpoint reachable (mutual TLS)** rather than a failure.
...
**Outbound routing.** For outbound trunks, choose how calls leave Bland:

- **Via Bland's carrier (default)** — outbound calls are placed through Bland's upstream carrier.
- **Direct to your carrier (BYOC)** — outbound calls bridge straight to your own SIP carrier and Bland answers the carrier's digest challenge natively. Requires SIP credentials (`sip_username`, `sip_password`) on the trunk; the wizard shows a carrier-side checklist and warns if credentials are missing. See [Direct carrier egress (BYOC)](#direct-carrier-egress-byoc) for details.
...
### Direct carrier egress (BYOC)

By default, outbound calls placed through Bland are routed via Bland's upstream carrier. If you have your own SIP carrier — such as **Bandwidth**, **Telnyx**, or any provider that supports standard SIP digest authentication — you can bridge outbound calls **directly** to that carrier from Bland's SIP edge instead.

To enable direct carrier egress on a trunk, set your carrier credentials in the outbound `options` when creating or updating it:

- `sip_username` — the SIP username your carrier expects
- `sip_password` — the SIP password your carrier expects
- `sip_realm` *(optional)* — pin the digest realm. Leave unset to accept whatever realm the carrier returns in its `401` / `407` challenge (the common case).

Once these are set, outbound calls placed on numbers attached to that trunk are bridged straight to your carrier and Bland answers the carrier's digest challenge natively — no Twilio hairpin.

```json Example (attach with direct carrier credentials)
{
  "phone_numbers": ["+14150000000"],
  "directions": [
    {
      "type": "outbound",
      "sip_endpoint": "sip:your-carrier.example.com",
      "options": {
        "transport": "tls",
        "secure_media": true,
        "sip_username": "your-carrier-username",
        "sip_password": "your-carrier-password",
        "sip_realm": "your-carrier.example.com"
      }
    }
  ],
  "service": "sip",
...
Direct egress is available for outbound (`outbound` or `both`) trunks. Inbound routing is unchanged.
