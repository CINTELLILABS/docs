...
### Cold transfers to a SIP or DID target

When an agent transfers a call to a SIP or DID target on one of your trunks and SIP REFER is turned off, Bland's SIP edge dials your SBC directly over TLS. The INVITE arrives from the signaling and media IPs listed by [`GET /v1/sip/firewall-ips`](/api-v1/get/sip-firewall-ips), so no additional allow-list entries are required as long as your SBC admits all IPs returned for your region.

The custom outbound SIP headers configured on the trunk are included on that INVITE, so downstream systems (PBX, call routing, analytics) see the same header block they receive on trunk-originated outbound calls. Warm transfer consult legs to a SIP target also carry these headers.

Transfers using SIP REFER, warm transfers merged in Bland, and transfers to regular phone numbers are unchanged.
