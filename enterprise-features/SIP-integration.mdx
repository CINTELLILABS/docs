...
If a trunk or BYOC number isn't yet registered in your account, use the **"Add a number you own"** input to add it directly. Enter the number in E.164 format (for example, `+14155551234`) — if you omit the country code, US (`+1`) is assumed. The number is validated and appears as a removable tag alongside your searchable account numbers. Duplicates and invalid entries are rejected with an inline error.
