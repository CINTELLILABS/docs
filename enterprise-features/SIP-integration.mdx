...
curl -X POST https://api.bland.ai/v1/sip/attach \
  ...
  -d '{
    ...
    "directions": [
      ...
      { 
          "type": "outbound", 
          "sip_endpoint": "sip.customer.com", // FQDN Only. Prefixed with `sip:`
          "options": {
...
<ParamField query="phone_number" type="string">
  The phone number to verify SIP calling from. It must be a attached to your organization first before attempting to verify.
</ParamField>
...
curl -X POST https://api.bland.ai/v1/sip/update \
  ...
  -d '{
    ...
    "updates": {
      "type": "outbound",
      "sip_endpoint": "sip:user@your.provider.com" // Only available for outbound SIP Types.
