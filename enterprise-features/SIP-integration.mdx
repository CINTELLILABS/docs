...
Outbound trunk calls egress through the carrier, so a full packet-level trace isn't captured. For these calls, the ladder shows a synthesized dispatch and outcome timeline (dial → carrier response → final status) so you can still see how each attempt resolved. Both inbound and outbound trunk call outcomes are included in the trunk's call log and answer-rate analytics.
