...
**[2] Idenifying your Numbers**

Bland needs to know ahead of time, the numbers that will be used in either an `Outbound` (also known as as `Origination`) direction, or an `inbound` (also known as `Termination`) direction.
...
This can be acompolished in two ways:
- [Configure them visually, on the dashboard.](https://app.bland.ai/dashboard/sip)
...
To make an outbound call with a SIP number, you simply use them like a regular `from` number. We'll automatically use your SIP information to connect your chosen SIP endpoint, and we'll make the call.
