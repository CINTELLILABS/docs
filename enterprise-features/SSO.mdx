...
- An **enterprise** Bland organization (SSO is available on enterprise plans only)
- An **owner** or **admin** role in that organization — only owners and admins can add or update SSO providers
...
When you register a provider, Bland validates the email domain you claim:

- Enter a single domain, such as `company.com`. URLs, comma-separated lists, and bare TLDs are rejected. Domains are stored lowercased.
- Shared mailbox domains (for example `gmail.com` or disposable email domains) cannot be used. Use a domain your organization owns.
- Each domain can be claimed by only one organization. This includes subdomains: if another organization has claimed `company.com`, you cannot claim `company.com` or `sub.company.com`.

A registration or domain update that fails these checks is refused with a clear error message. See the [error code reference](#appendix-error-code-reference) below.

<Note>
Your own organization can register multiple providers on the same domain (for example, OIDC and SAML side by side). If your domain is already claimed by another organization, contact support@bland.ai.
</Note>
...
<Info>
Contact support@bland.ai for custom role mapping configuration.
</Info>
...
- Each email domain (including its subdomains) can only be configured for one organization
...
<AccordionGroup>
  ...
  <Accordion title="Can I have multiple SSO providers?">
    ...
    Each domain belongs to a single organization. Your organization can run more than one provider on the same domain (for example, OIDC and SAML side by side).
  </Accordion>
  ...
  <Accordion title="What if my domain is already registered?">
    Contact support@bland.ai to transfer or reset domain ownership.
  </Accordion>
...
| `SSO_ORGANIZATION_REQUIRED` | No organization specified during registration | Select your organization and try again |
| `SSO_ENTERPRISE_REQUIRED` | Organization is not on an enterprise plan | Contact your Bland account team to enable SSO |
| `SSO_ORG_ADMIN_REQUIRED` | Caller is not an owner or admin | Ask an organization owner or admin to manage SSO providers |
| `SSO_DOMAIN_INVALID` | Domain is not a single valid hostname | Enter one domain, such as `example.com` |
| `SSO_DOMAIN_NOT_ALLOWED` | Domain is a shared mailbox domain | Use a domain your organization owns |
| `SSO_DOMAIN_CLAIMED` | Domain already claimed by another organization | Contact support@bland.ai to transfer domain ownership |
| `SSO_PROVIDER_CONFLICT` | Multiple organizations have verified providers for the domain | Contact support@bland.ai |
