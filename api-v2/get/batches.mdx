...
<ParamField query="agent_id" type="string">
  Optional. UUID of a staffing agent. When provided, only batches created with that `global.agent_id` are returned. Non-UUID values return a 400 with `BATCH_V2_LIST_VALIDATION_ERROR`.
</ParamField>
