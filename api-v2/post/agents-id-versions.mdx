...
<ParamField body="snapshot" type="object" required>
  ...
  <Accordion title="knowledge (optional)">
    When present, must be `{ "kbIds": string[] }`, the knowledge base item IDs the agent retrieves against on every call. `null` is rejected; omit the key instead. Order carries no meaning.
  </Accordion>
</ParamField>
