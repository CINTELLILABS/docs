...
<ParamField body="agent_id" type="string">
  Optional UUID of the agent to scope this tool to. When set, the tool is only returned by `GET /v2/tools` requests that filter by the same `agent_id`. Omit this field to create a tool that isn't tied to any agent.

  Attribution is set at create time and can't be changed later — subsequent updates to this tool ignore `agent_id`.
</ParamField>
