...
<ParamField body="global" type="object" required>
  ...
  **Required keys:**
  ...
  - Must include at least one of: `"task"`, `"pathway_id"`, or `"agent_id"`. \
    \
    **Forbidden keys:**
...
<ParamField body="global.agent_id" type="string">
  Optional. UUID of an agent that staffs every call in the batch. The agent supplies voice, model, and behavior; pair it with `global.task` to add per-batch instructions. Requires an organization API key. Cannot be combined with `pathway_id` or `persona_id` in `global`.

  ```json Example
  "global": {
    "agent_id": "b1c2d3e4-5678-90ab-cdef-1234567890ab",
    "task": "Confirm the customer's appointment for tomorrow at 2pm."
  }
  ```
</ParamField>
...
When `global.agent_id` is present, requests are rejected up front with a 400 and one of the following error codes if the agent staffing check fails:

| Code | Meaning |
|------|---------|
| `AGENT_ID_INVALID` | `agent_id` is not a valid UUID. |
| `AGENT_PARAM_CONFLICT` | `agent_id` was sent alongside `pathway_id` or `persona_id`. |
| `AGENT_ORG_REQUIRED` | The request is not authenticated with an organization API key. |
| `AGENT_NOT_FOUND` | No active agent with that ID belongs to your organization. |
