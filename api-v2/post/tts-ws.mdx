...
<ParamField body="text" type="string" required>
  ...
  Pause markers (<code>&lt;|N|&gt;</code>, N in seconds) are accepted and may straddle two deltas. A marker above 10 seconds, or an opening <code>&lt;|</code> that is not closed within 16 characters, sends an `invalid_request` error and ends the turn as `failed`.
</ParamField>
...
| `invalid_request` | Message, turn, or session | A field is invalid for the current state, or an `N`-second pause marker exceeds 10 seconds (the turn ends as `failed`). |
