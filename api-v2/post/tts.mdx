...
Text-to-speech is billed per character of `text` against your wallet balance. The base rate is **\$0.05 per 1,000 characters**. An organization with an active Speech subscription is charged its tier's discounted per-character rate instead, on all of its usage. Each request carries a minimum charge of \$0.001, so very short generations bill at the minimum rather than the per-character rate.
...
<ParamField body="text" type="string" required>
  ...
  Insert a pause with <code>&lt;|N|&gt;</code>, where N is a positive number of seconds, at most `10`: <code>Welcome to Bland. &lt;|0.8|&gt; How can I help?</code>. A marker above 10 seconds returns `400 invalid_request` naming the marker; use several shorter pauses instead.
</ParamField>
...
| Code | HTTP | Meaning |
|---|---|---|
| `invalid_request` | 400 | A required field is missing or has the wrong shape, or an `N`-second pause marker exceeds 10 seconds. |
...
| `concurrency_limit_reached` | 429 | Your organization has no free speech concurrency slot. `Retry-After` is set; see [Speech Limits](/speech/limits). |
