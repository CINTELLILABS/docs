...
<ParamField body="text" type="string" required>
  The text to speak. Maximum 5,000 characters, and it must contain at least one letter or digit: punctuation, symbols, or emoji alone produce no audio and return `400 invalid_request`. If you are forwarding LLM tokens one request at a time, use [Realtime Speech (WebSocket)](/api-v2/post/tts-ws) instead, which buffers deltas into speakable chunks for you.
