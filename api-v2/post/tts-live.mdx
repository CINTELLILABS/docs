...
| `insufficient_credits` | Your account is out of credits. Sent at connection, or before a flush if the balance runs out mid-session. Buffered text is preserved, so a top-up followed by another `flush` renders it. |
