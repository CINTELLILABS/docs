...
<ParamField body="parity" type="boolean" default="false">
  When `true`, produces an agent that handles calls exactly like the original pathway: the call opens on the pathway's own first step (no extra greeting or re-routing), End Call steps still hang up in the same place, and the phone number's call settings (voice, language, interruption sensitivity, recording, max duration) carry over from the pathway's bound inbound number. Use this for a guaranteed 1:1 behavioral match; the default import optimizes the resulting agent for the v2 builder instead.

  Valid only when `pathways` has exactly one entry and `create` is `true` (or omitted). Any other combination returns `400 INVALID_PATHWAYS`. Anything that can't carry over exactly is added to `data.warnings` for review in the builder.
</ParamField>
