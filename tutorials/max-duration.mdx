...
<img style={{ borderRadius: "0.5rem" }} src="/tutorials/tutorials-assets/maxduration.jpg" />
...
### Behavior on transferred calls

On self-serve accounts, `max_duration` bounds the entire call, including any leg that has been cold-transferred off the AI agent. If a call is transferred and the remaining budget runs out, the transferred leg is ended and the call closes normally.

Accounts on dedicated infrastructure are unchanged. There, `max_duration` bounds only the AI agent's portion of the call, and a transferred leg can run longer than the configured limit. Teams that rely on this to set a low `max_duration` as a stuck-agent guard do not need to change anything.
