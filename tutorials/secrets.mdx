...
* Headers (optional)
* Body (if using POST)
* Refresh interval (in minutes; between 10 and 1440 minutes — up to 24 hours)
...
#### Pick the JSON path visually

When configuring a refreshable secret, you can run the request once from the dialog to preview the response and click the value you want to extract. The picker writes the matching JSON path into the **Extract Secret From Response Body** field for you, so you don't have to hand-write the path.

Values in the preview are masked — only the shape of the response is shown, never the underlying data.
...
Clicking `access_token` in the preview sets the path to:
...
* Secret keys may only contain letters, digits, and underscores — no spaces, dashes, or other special characters.
