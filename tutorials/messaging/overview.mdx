...
<CardGroup cols={2}>
  ...
  <Card title="One API surface" icon="code">
    Send and receive across every channel with the same `/v1/sms/*` endpoints. Channel is configured per number or selected per request.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="WhatsApp" icon="whatsapp" href="/tutorials/messaging/whatsapp">
    Conversations on the world's most used messaging app. Available to organizations with WhatsApp enabled.
  </Card>
</CardGroup>
...
All channels share the same endpoints. Pass `channel` in the request body, or let it be auto-derived from the number's configuration.
...
- [WhatsApp](/tutorials/messaging/whatsapp) (per-number WhatsApp configuration for entitled organizations)
