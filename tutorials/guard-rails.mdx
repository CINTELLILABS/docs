...
1. **Agents** - under **Build** -> **Agent** -> **Guardrails** in the agent builder
2. **Personas** - under "General" -> "Policy & Compliance"
3. **Pathways** - under "Policy & Compliance" in the pathway splash page
4. **Inbound numbers** - under "Advanced" -> "Guard Rails"
5. **Any outbound call via API** - including the `guard_rails` field in the POST `/v1/calls` [API](/api-v1/post/calls#param-guard-rails)
...
## Guardrails in the agent builder

Agents built in the agent builder carry their own guardrails. Open an agent and go to **Build** -> **Agent** -> **Guardrails**. Nothing is enabled by default.

From the Guardrails tab you can:

- **Switch on the built-in TCPA rules** - AI disclosure, recording disclosure, self-introduction, and opt-out. The timed rules take a window (30 seconds of call start by default), and the agent must satisfy the rule within that window.
- **Write your own rule in plain English** - Select **Add guardrail** and describe what the agent must never do. The rule is evaluated on every agent turn during the call. Custom guardrails are an Enterprise feature, capped at five per agent.
- **Set actions per guardrail** - Each guardrail carries its own actions: end the call, transfer to a phone number, or move to a step in the agent. Actions run automatically when the rule is broken.

Guardrails are saved with the agent, so they version, branch, publish, promote, and roll back exactly like the rest of its configuration.

The editor rejects actions that cannot run. A transfer without a phone number blocks the save, and **Move to a step** only offers steps a call can actually move onto.
