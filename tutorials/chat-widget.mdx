...
## Manage widgets from your agent

You can also create and manage web chat widgets directly from an agent in the dashboard. Open your agent, go to **Channels**, and select the **Web chat** channel. Each agent can have multiple widgets, so you can run separate embeds for different sites or environments.

Select **New widget** to open the widget editor. The editor is split into two tabs:

- **Configuration**: the widget's backing (a prompt or a pathway), allowed domains, message rate limit, webhook URL and webhook mode, and inactivity timeout settings with optional warning and ending messages.
- **Appearance**: the header text or logo, launcher icon, accent color, footer visibility, and widget size. A live preview updates as you make changes.

Widgets backed by a pathway also get a **Components** tab for attaching rich blocks to pathway nodes. See [Custom Components](#advanced-custom-components).

### Publish a widget

New widgets start as drafts. A draft widget doesn't load on any site, so an unfinished embed is never live. To publish:

1. Add at least one allowed domain on the **Configuration** tab. Publishing requires it, because the widget only loads on domains in this list.
2. Select **Publish**.

Once published, the widget is live on its allowed domains and the embed snippet in the editor loads it. Select **Unpublish** to take the widget offline. Your domain list is preserved, so republishing restores it.
...
When using pathways in your widget, you can create custom components that display rich blocks in the conversation at specific pathway nodes, instead of plain text only.
...
You can attach five kinds of components to a pathway node:

| Type | What it shows |
|------|---------------|
| Quick Replies | A row of buttons. Each button either sends a user message or opens a redirect URL. |
| Accordion | Expandable groups of items the visitor can click to respond. |
| Cards | Markdown cards with a title, description, image, and actions. |
| Scheduling | A time-slot picker with a configurable timezone. |
| Iframe | Your own web content embedded in the chat window. Iframe URLs must use `https`. |

### Creating components in the dashboard

Author components from the **Components** tab of the widget editor:

1. Open your agent, go to **Channels**, and select the **Web chat** channel.
2. Open a widget. On the **Configuration** tab, set **Backed by** to **Pathway** and pick a pathway. Components fire when the conversation reaches a pathway node, so a prompt-backed widget can't show them.
3. On the **Components** tab, add a component, choose its type, and bind it to a pathway node.

For each component you can:

- **Block text input**: hide the chat composer while the component is on screen, so the visitor answers through the component.
- **Dynamic config**: supply the component's contents at runtime from a pathway variable (for example `{{welcome_card}}`) instead of writing them out in the dashboard.

You can also manage components via the [widget custom components API](/api-v1/post/widget-custom-components).
...
- Through the variables list on an iframe component in the widget editor
- Via the `variables` field in the [widget custom components API](/api-v1/post/widget-custom-components)
