...
| `persist_open` | boolean | false | If true, the widget remembers whether it is open and restores chat mode across same-origin page loads in the current browser tab. Voice returns to the channel menu. A restored open widget emits `bland_widget_state_change` when state events are enabled |
