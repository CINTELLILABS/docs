...
If a pathway node has media attachments configured, the widget can display those images directly in the chat. After the agent replies from that node, Bland picks the images whose "When to use" description matches the reply and appends them to the same chat bubble.

### Setup

<Steps>
  <Step title="Open the node in your pathway">
    In the pathway editor, select the node where you want images to be available. Both Default nodes and Knowledge Base nodes are supported.
  </Step>
  <Step title="Add media attachments">
    In the **Media attachments** section, upload your images. Give each image a name and a **When to use** description, for example "Send when the user asks about pricing or product details".
  </Step>
</Steps>

### How it works

- The agent's reply text is sent first. Image selection runs afterward, so it never delays the reply.
- Images are served through short-lived signed links that are regenerated on every page load, so images persist across page refreshes.
- Each image is shown at most once per conversation thread.
- Message content stays plain text. Message webhooks, live agent transcripts, and dashboard logs are unchanged.
- SMS and RCS delivery of node media uses the same "When to use" selection rules and is unaffected.
