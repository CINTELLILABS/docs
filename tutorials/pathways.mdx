...
For inbound SIP calls, you can hand the call off at the SIP layer instead of bridging a new outbound leg. On the Transfer Call node, enable **SIP REFER** to perform the cold transfer using a SIP REFER message. Bland then exits the media path cleanly once the destination accepts the call.

**When to use it:**

- The inbound call arrived over your SIP trunk (see [SIP Integration](/enterprise-features/SIP-integration)).
- You want a true cold transfer — no warm hand‑off, no AI on the line after transfer.
- You want your carrier (rather than Bland) to carry the resulting call leg.

**How to enable it:**

- In the pathway editor, open the Transfer Call node and toggle **Enable SIP REFER**.
- Set the transfer destination as you normally would.

The transfer target does not need to be a pre‑registered outbound SIP number. If it isn't, the call is routed over your organization's outbound SIP trunk automatically.

<Note>
SIP REFER only applies to cold transfers on inbound SIP calls. For non‑SIP calls, or when SIP REFER is disabled, the Transfer Call node falls back to the standard transfer behavior. Warm transfers are unaffected — see [Warm Transfer](/tutorials/warm-transfer).
</Note>
