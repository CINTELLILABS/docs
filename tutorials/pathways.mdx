...
See the [Pathways API reference](/api-v1/post/pathways) to create and manage pathways programmatically.

## Pathway Versions

A pathway is the container your phone numbers and API calls point at. Its ID stays the same for its whole life. The flow itself (the nodes and edges) lives in versions: snapshots of the pathway at a point in time.

When you edit a pathway, you are working on a draft. Live calls are not affected. They keep using the published production version. When you are happy with your changes, publish the draft. This saves it as a new version and promotes it to production, and new calls pick it up immediately.

You can also promote a version to staging to test it before it goes live, or send an individual call against a specific version by passing its version number. By default, calls use the production version.

See the [Pathway Versions API reference](/api-v1/get/pathway_versions) to manage versions programmatically.
...
### Agent speech for End Call and Cold Transfer tools

Bland **End Call** and **Cold Transfer** tools attached to a node can speak a final line before they act. In the node's tool settings, the section appears as **Before the call ends** (End Call) or **Before the transfer** (Cold Transfer), with an **Agent speech** field and an **AI-composed** switch.

- **AI-composed off**: the agent speaks the line verbatim. For example, `Thanks for calling, goodbye.`
- **AI-composed on**: you write an instruction instead of a fixed line. The agent composes one line from the conversation before the call ends or transfers. For example, the instruction `Thank the caller by name and say goodbye` produces a closing line tailored to that call.

In AI-composed mode, you can also set an optional **Fallback line**. The agent speaks the fallback verbatim if composing fails. An AI-composed instruction cannot be empty: applying without one shows an inline error. To use a fixed line only, switch AI-composed off.

## Transfer Call Node

The transfer call node is used to transfer the call to another destination when the node is reached, and the dialogue at this node is complete.
...
Open the **Transfer type** dropdown to pick how the call is handed off:

- **Phone Number** — Transfer to a standard E.164 phone number. The input is validated as a phone number, and you can toggle **Use Variable** to pass the destination in at call time.
- **DID / SIP target** — Transfer to a SIP destination such as a DID, dial-plan extension, or a SIP URI like `sip:user@host`. The input is a free-form text field with the placeholder `DID, extension, or sip:user@host`, and E.164 phone-number validation is skipped so non-PSTN targets are accepted. Use this option when routing the call through SIP infrastructure — for example, alongside a configured [SIP trunk](/enterprise-features/SIP-integration).
- **Twilio App** — Transfer the call to a Twilio TwiML application (available when you have a connected Twilio account).

Choose **Phone Number** for regular PSTN handoffs, and **DID / SIP target** when the destination is not a normal phone number — for example a SIP URI, an internal extension, or a DID that should not be reformatted to E.164.
...
- `{{voice_id}}` — The ID of the voice active on the call. This is most useful with voice pools, where the voice is selected at call time: pathway logic can branch on which voice is active (for example, to use voice-dependent names or prompts). The value is the same voice ID you added to the voice pool, so you can match against it directly. If you pass your own `voice_id` request variable, that value takes precedence.
