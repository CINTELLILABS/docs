...
- `{{memory_injected}}` — Only set on calls that use `memory_id`. `true` when legacy memory was found and injected into the call context, `false` when no memory payload was available (for example, the contact has been migrated off the legacy memory store). Unset for calls that don't use memory. Use this to deterministically branch or apply guardrails based on whether memory context is actually present, instead of asking the model to infer it.
