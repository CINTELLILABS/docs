...
#### Voice cloning on warm transfer

When [BTTS v2 voice cloning](/tutorials/btts) is enabled for your organization, you can have the proxy agent speak to the live operator in a cloned voice during the briefing. This makes the handoff sound consistent with the rest of the call rather than switching to a different stock voice.

**Where to find it**: On a Transfer Call node, enable Live Translation. A new **Enable voice cloning** toggle appears in the same section.

**What it does**: When enabled, the proxy agent plays a short calibration prompt to the operator at pickup. This lets the system calibrate the cloned voice before the briefing begins. The operator's acknowledgement is used as the calibration signal, so the prompt should ask for a brief spoken reply.

**Calibration prompt**: You can customize what the proxy agent says to the operator. If you leave the field empty (or whitespace-only), Bland uses a sensible default prompt. A live preview shows what the operator will hear and the expected acknowledgement.

**Configuration**:
- **Default**: Disabled
- **Visibility**: The toggle only appears when your organization has BTTS v2 voice cloning enabled and Live Translation is on for the node
- **Calibration prompt**: Optional — leave blank to use the default

<Tip>
Keep the calibration prompt short and direct (one sentence). The operator only needs to say a few words back for the clone to calibrate before the briefing.
</Tip>
