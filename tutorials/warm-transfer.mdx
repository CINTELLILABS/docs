...
If the agent doesn't answer or is unavailable, the system can [retry the agent](#11-transfer-retry) before falling back to voicemail handling.
...
**What it does**: Automatically redials the human agent when an attempt fails, while the customer stays on hold.

**Why it matters**: Agents often miss the first call because they're on another line or away from their phone. A quick redial connects transfers that would otherwise fail after a single attempt.

**How it works**: When an attempt ends with an outcome you've opted into, the system waits for your configured delay and dials the same agent again. The customer keeps hearing hold music between attempts, and each redial uses the same caller ID as the first attempt. Retries stop when an attempt connects, the retry limit is reached, or the hold timeout expires. The [hold timeout](#9-hold-timeout) is the total budget for the transfer, so it caps time across all attempts. Once attempts are exhausted, the transfer falls back to your [voicemail handling](#10-voicemail-message-configuration).

**Location**: Transfer Call node → warm transfer settings → Toggle "Retry if the agent doesn't pick up"

**Configuration**:
- **Default**: Disabled (one attempt per number)
- **Additional attempts**: 0 to 4 extra dials after the first attempt
- **Seconds between attempts**: 0 to 60 seconds; 0 redials right away
- **Retry when the attempt ends in**: Any combination of voicemail, no answer, busy, or carrier failure. Select at least one, or the agent is never redialed automatically.
- **Let the proxy pathway request a redial**: Honors a Retry Transfer node in the proxy agent's pathway. Available only when the briefing step uses a pathway.

<Note>
An agent who answers and hangs up without merging is never automatically retried, since the system can't tell a call screener from an agent who declined the transfer. Use the Retry Transfer node to handle screeners.
</Note>

#### Retry Transfer node

If your proxy agent runs a pathway, you can add a **Retry Transfer** node to it from the pathway editor's node menu. When the pathway reaches this node, the agent speaks an optional closing line (for example, "Thanks, I will try again in a moment"), ends the current attempt, and requests a redial from the parent transfer. The node is terminal, so it has no outgoing edges.

**When to use it**: Route to a Retry Transfer node when the proxy agent reaches a call screener, receptionist, or automated attendant instead of the intended agent. Since these calls count as answered, they won't trigger an automatic retry on their own.

**Requirements**:
- **Let the proxy pathway request a redial** must be enabled in the Transfer Call node's retry settings
- The retry limit must not be exhausted

If either condition isn't met, or the node is used outside a warm transfer proxy pathway, it behaves like an End Call node.
...
Each attempt to reach an agent has its own state in the `proxy_agent_calls` array. These provide detailed insight into individual agent connection attempts. When [transfer retry](#11-transfer-retry) is enabled, each redial appears as its own entry in the array.
...
For additional support, reach out to your customer success representative, our [support email](mailto:support@bland.ai), or [Discord](https://discord.gg/QvxDz8zcKe).
---
