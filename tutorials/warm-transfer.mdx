...
**What it does**: Plays a short static message to the live agent if the customer hangs up mid-transfer, before the calls have merged.

**Why it matters**: Without this message, the agent leg is dropped silently when the customer disconnects during the briefing phase. A spoken notice lets your agent know the customer left, so they aren't left wondering why the line went quiet.

**How it works**: If the caller ends the call after the proxy agent answers but before the merge completes, Bland plays this message to the agent and then ends the agent leg. Leave the field blank to keep the legacy behavior of silently dropping the agent.

**Configuration**:
- **Input**: Plain text message
- **Default**: Empty (silent drop, legacy behavior)
- **Example**: "The caller has disconnected before we could connect you. You can hang up now."
