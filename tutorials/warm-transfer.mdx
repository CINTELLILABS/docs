...
#### Voice cloning for translated speech

When live translation is enabled, you can also turn on **Voice Cloning** so the live operator hears the translated speech rendered in the customer's own voice instead of a generic synthesized voice. Bland captures a short sample of the customer's natural speech while they talk to the AI, generates an ephemeral voice clone, and uses that clone for the translated audio the operator hears once the call is merged.

**When to use it**: Multilingual support flows where you want the operator to retain a sense of who they're talking to (tone, identity, vocal cues) instead of every translated caller sounding the same.

**How it works**:
- Cloning kicks off in the background during the AI conversation, so it doesn't delay the transfer.
- If the customer hasn't spoken long enough to produce a usable clone, the system silently falls back to the standard translation voice — the call still completes normally.
- The clone is ephemeral: it's used only for the duration of the transfer and isn't saved to your voice library.

**Configuration**:
- **Location**: Transfer Call node → Live Translation section → Enable Voice Cloning
- **Default**: Disabled
- **Requirement**: Live Translation must be enabled on the same node

<Note>
This is the first half of bilateral voice cloning (customer-side). Operator-side cloning — so the customer hears the operator in the operator's own voice — is planned for a future release.
</Note>
