...
When Live Translation is on and your organization has voice cloning enabled, you can have each participant hear the other speaking in a clone of their own voice instead of a generic translation voice. This makes the merged call feel more natural — the customer hears the agent in the agent's voice, and the agent hears the customer in the customer's voice, just in their respective languages.

**How it works**: When the operator picks up, the proxy agent plays a short calibration prompt and captures a brief sample of the operator's speech to generate a short-lived voice clone. The same is done for the customer. From that point on, translated audio is rendered in each participant's cloned voice.

**Configuration**:
- **Enable voice cloning**: Toggle on under the Live Translation section. Off by default.
- **Voice cloning calibration prompt**: Optional. What the proxy agent says to the operator at pickup to elicit a speech sample. Leave empty to use the default prompt.
- **Availability**: Only appears if voice cloning is enabled for your organization. [Contact our team](https://www.bland.ai/book-a-demo) if you don't see the option.

<Tip>
Keep the calibration prompt short and conversational — you only need a few seconds of natural speech to produce a usable clone. A good prompt asks the operator to introduce themselves or briefly state what they can help with.
</Tip>
