...
- **Max retries**: 0 to 4 additional dials after the first attempt
- **Retry delay**: 0 to 60 seconds between attempts
- **Retry on**: Any combination of voicemail, no answer, busy, or carrier failure
- **Allow pathway retry**: Lets a Retry Transfer node in the proxy agent's pathway request a redial
...
If your proxy agent runs a pathway, you can add a **Retry Transfer** node to it. When the pathway reaches this node, the agent speaks the node's dialogue, ends the current attempt, and requests a redial from the parent transfer.
...
**Requirements**:
- **Allow pathway retry** must be enabled in the Transfer Call node's retry settings
