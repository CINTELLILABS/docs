...
### 11. Early Hangup Message

**What it does**: Plays a short message to the live agent if the caller hangs up after the agent has already answered, but before the calls are merged.

**Why it matters**: Without this, the agent's call is dropped immediately when the caller disconnects mid-briefing, leaving them with confusing dead air. A brief explanation lets the agent know what happened before the call ends.

**How it works**: When the caller hangs up while the proxy agent is on the line in the pre-merge window, the agent hears your configured message and then the call ends cleanly. If the caller hangs up before the agent answers, or after the calls have already merged, this message is not played.

**Configuration**:
- **Default**: Empty (immediate hangup — preserves the existing behavior)
- **Format**: Plain text message spoken to the agent
- **Example**: `Sorry — the caller hung up before we could complete the transfer. You can end the call now.`

<Tip>
Leave this field blank if you want the agent's call to end immediately when the caller drops. Set it whenever your live agents have reported confusion about silent disconnects mid-briefing.
</Tip>
