...
If you're using SIP trunking and want to hand the call off natively instead of bridging a new outbound leg, you can enable **SIP REFER transfer** on the Transfer Call node.

**What it does**: Transfers the call using a SIP REFER message sent to your trunk, so the downstream system takes ownership of the call leg instead of Bland bridging a new outbound call.

**When to use**: Enable this for SIP trunk customers who want their carrier or PBX to handle the final transfer destination. It only applies to cold transfers on SIP calls.

**Location**: Transfer Call node → Toggle "SIP REFER transfer"
**Default**: Disabled
**Visibility**: Only shown when warm transfer is disabled. Enabling warm transfer hides and clears this setting.

<Note>
SIP REFER requires that your SIP trunk and downstream switch support the REFER method. If the trunk doesn't accept the REFER, the transfer will fail.
</Note>
