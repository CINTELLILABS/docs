...
**Voice matching**: Translated audio is played back in a live clone of the original speaker's voice. The customer hears the agent's translations in a voice that matches the agent, and the agent hears the customer's translations in a voice that matches the customer, so each party sounds like themselves across languages. No setup is required — cloning happens automatically when live translation is enabled.
