...
In the Actions panel, click **POST** (or **GET**, **PUT**, **PATCH**, or **DELETE** depending on your API) to open the Tool Builder. You can also change the method later from the request editor.
...
Configure the request body by adding variables. Each variable becomes a field in your API request. The Body tab is only shown for methods that support a request body (POST, PUT, and PATCH). It's hidden for GET and DELETE.
...
- **Audio:** Extract this variable from the caller's audio instead of the transcript (see [Audio extraction](#audio-extraction) below)
...
By default, the agent extracts variable values from the call transcript. Turning on the **Audio** switch on a variable tells the agent to extract that value directly from the caller's audio instead, capturing spoken nuances that text transcripts can miss.

Use audio extraction when transcript accuracy matters and small errors are costly, such as:

- Order numbers, confirmation codes, or reference IDs
- Account numbers, ZIP codes, or numeric identifiers spoken digit by digit
- Names or terms that transcription models frequently mishear

You can enable audio for any subset of variables in the same tool — leave transcript-based extraction on for free-form fields (summaries, notes) and turn audio on only for the fields where precision matters. The Audio switch is only available on agent-filled variables; variables with fixed or secret values aren't extracted from the call, so the toggle doesn't apply to them.

The setting applies both when the tool runs in a Persona and when it's attached to a pathway node. The same per-variable **Audio** toggle is also available on custom code (snippet) tool variables and behaves the same way. If no audio is available for the call (for example, chat sessions), the agent automatically falls back to transcript extraction for that variable, so tools stay functional across all channels.
