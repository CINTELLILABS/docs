...
By default, the agent extracts variable values from the call transcript. Turning on the **Audio** switch on a variable tells the agent to extract that value directly from the caller's audio instead.
...
If no audio is available for the call (for example, chat sessions), the agent automatically falls back to transcript extraction for that variable, so tools stay functional across all channels.
