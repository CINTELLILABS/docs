...
The setting applies both when the tool runs in a Persona and when it's attached to a pathway node. If no audio is available for the call (for example, chat sessions), the agent automatically falls back to transcript extraction for that variable, so tools stay functional across all channels.
