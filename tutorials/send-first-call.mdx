...
description: "Send your first AI phone call with Bland in under 2 minutes (redirects to the API reference)."
---

<img style={{ borderRadius: "0.5rem" }} src="/tutorials/tutorials-assets/sendfirstcall.jpg" />
...
- Set parameters like `reduce_latency`, `record`, `ivr_mode` (for navigating phone trees), and `wait_for_greeting` according to your call's requirements.
...
3. Choosing the language with the `language` parameter.
4. Setting a `max_duration` for the call.
