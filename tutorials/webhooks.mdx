...
- Bearer: Use this option to authenticate with a bearer token. Enter the raw token — the `Bearer` prefix is added automatically.
- Basic: Authenticate with a username and password. Enter `username:password` in the token field and check **Encode** to have it base64-encoded for you, or paste an already base64-encoded value and leave **Encode** unchecked. Enabling **Encode** on a pre-encoded value double-encodes it and the request fails with a 401.

Headers and auth tokens can reference [Bland Secrets](/tutorials/secrets) for secure credential management.

This same Authorization configuration applies to the Webhook node, the Scheduling node's available-slots fetch, and webhook-style tools configured on a node.
...
<AccordionGroup>
  <Accordion title="Built-in Variables">
    ...
    - `{{timezone}}` - The timezone of the other party’s `{{phone_number}}` (ex. America/New_York).
...
> **Important**: Routing based on response HTTP codes can only be done if Response Data is enabled. If response is disabled, the webhook will act asynchronously because no response is expected.
