...
- **Credential values are withheld** before storage. Webhook events keep the full request URL (with query parameters) and your configured headers for debugging, but values that look like credentials — API keys, tokens, signatures — are replaced with a placeholder. You may see similar placeholders in variable values.
