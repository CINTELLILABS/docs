...
| Event | Field | Type |
|---|---|---|
| invoke | `input_vars` | `object` (optional) — the resolved input variables the code ran against |
| invoke | `code_ref` | `string` (optional) — which snippet ran (older calls only) |
...
`input_vars` contains only the variables the node was configured to pass in, with placeholders resolved to their runtime values. Values that come from the secret store read as `[REDACTED]`; ordinary pathway variable values are shown as-is. Omitted when the node configures no inputs. Present on newer calls only.
