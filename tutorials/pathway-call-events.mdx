...
| `node.transition` | Every routing decision: which node was chosen, from where, which routes were available, and what variables changed crossing the edge. |
| `node.tag` | A tag attached to a node the call passed through. These aggregate into the call's pathway tags. If the start node is tagged, its `node.tag` is the first tag event of the call, immediately after `conversation.init`. |
...
| `available_routes` | `array` (optional) | The routes the agent could have chosen for this transition, snapshotted at decision time. Entry shape below. Absent on older calls — absence means "no data recorded", never "no options". |
| `routes_truncated` | `boolean` (optional) | `true` when the candidate list was capped (30 max) — a short list should not be read as the full option set. |
| `chosen_candidate_index` | `number` (optional) | Zero-based index of the chosen route within `available_routes`. Use this rather than matching on label, which stays exact when two edges share a label. |
| `decision_source` | `string` (optional) | How the decision was made: `"llm"` (the routing model chose), `"deterministic"` (a value-comparison edge matched), `"single_route"` (the node had exactly one outgoing edge), or `"flex_llm"` (the flex router picked the next node). |
...
Each entry in `available_routes`:

| Field | Type | Notes |
|---|---|---|
| `label` | `string` | The edge's authored label. |
| `condition` | `string` (optional) | The authored condition on the edge, when one exists. |
| `always_pick` | `boolean` (optional) | The edge was configured to always be picked. |
| `is_global` | `boolean` (optional) | The edge comes from a global node rather than the current node. |
| `synthetic_wait` | `boolean` (optional) | Marks the "none of the above" option the runtime adds so the agent can decline every route and keep listening. Choosing it is a wait, not a routing failure. |

### `node.tag`

The payload is the tag itself: `{ "name": "...", "color": "#...", ... }`. `name` is always present; `color` and editor metadata may accompany it.

A tagged start node emits its `node.tag` directly after `conversation.init`, before any transcript or transition. Untagged start nodes emit nothing. Calls recorded before this behavior shipped do not include a start-node `node.tag`.
...
| `available_tools` | `array` (optional) | The tools offered for this generation, as `[{ "name": "..." }, ...]`, including built-ins like `Press Button` and `Wait`. Also present when the model chose to speak — declining every tool is itself a decision. Absent on older calls. |
| `tools_truncated` | `boolean` (optional) | `true` when the tool list was capped (25 max). |
...
| invoke | `candidates` | `array` (optional) | Every deterministic edge weighed for this evaluation, in authored order, as `[{ "target_node": "...", "condition": "..." }, ...]`. Captured before first-match-wins runs, so you can tell an edge that evaluated false from one the match short-circuited past. Absent on older calls. |
| invoke | `candidates_truncated` | `boolean` (optional) | `true` when the candidate list was capped (30 max). |
...
| result | `clauses` | `array` (optional) | Per-clause outcome of a structured condition, as `[{ "key": "...", "description": "...", "met": true }, ...]` — shows which clause held the whole condition back. Absent for text (LLM) conditions and on older calls. |
| result | `clauses_truncated` | `boolean` (optional) | `true` when the clause list was capped (30 max). |
...
- **Decision alternatives are newer fields.** `available_routes`, `available_tools`, `candidates`, and `clauses` are only recorded on calls placed after the feature shipped. Absence means the data wasn't recorded, never that the agent had no options. When a `*_truncated` flag is `true`, the list was capped and options beyond the cap are not persisted.
