...
<Note>
A node's setting applies only while that node is active. Moving to a node with no setting of its own reverts to the call-level (or inbound number-level) baseline, not to whatever the previous node had set.
</Note>
