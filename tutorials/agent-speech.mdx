...
- **Batch calls**: the batch form carries the same density, reactions, and never-say controls, applied to every call in the batch.
- **Personas**: the persona Behavior tab stores the policy on the persona, so any call that uses it inherits the same settings.
- **Triggers and Events**: call triggers and event-driven calls persist the policy on the call config and send it with each dispatched call.
- **Dashboard**: on the Voice channel's Conversation feel tab for your agent, use the Agent backchannels select for density, the Reactions switch for `continuers_only`, and the Block utterances input for `blocked_tokens`. Reactions and Block utterances appear once backchannels are on. The same three controls also show up on the Send Call form and the pathway test-call panel.
