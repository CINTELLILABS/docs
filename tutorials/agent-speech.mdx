...
<Warning>
Node settings are not scoped to the node. A setting takes effect from that node onward and stays in force for the rest of the call, until another node changes it. This is useful for shifting the agent's manner mid-call, for example becoming harder to interrupt while reading a legal disclosure and easier while gathering information. But it also means a node with no setting of its own inherits whatever an earlier node set: if callers are being talked over (or not heard) at one point in a pathway, check the nodes upstream of it, not just the node where it happens.
</Warning>
