...
<Note>
A node's setting applies only while that node is current. Moving to a node that does not declare its own value reverts to the call-level setting from your original call parameters. To keep a value across several nodes, set it on each of those nodes.
</Note>
