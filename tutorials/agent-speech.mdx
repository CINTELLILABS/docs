...
<Warning>
A node's setting stays in force for the rest of the call, not just that node. If the agent misbehaves at a node with no setting of its own, check the nodes before it.
</Warning>
