...
### Staffing a batch with an agent

In the new dashboard, every batch is staffed by one of your agents. The staffing agent supplies the voice, model, and behavior for every call in the batch. You can:

- Create a batch from the **Batch Calls** page and pick a staffing agent in the modal, or
- Open an agent's **Batches** tab to create a batch that is pre-locked to that agent.

Use the **Instructions** field on the create form to add per-batch guidance on top of the agent's normal behavior (for example, the specific script for that campaign).

To staff a batch via the API, pass the agent's UUID as `global.agent_id` on `POST /v2/batches/create` and use `global.task` for the per-batch instructions. See the [Create Batch reference](/api-v2/post/batches) for the full payload and error codes. Agent staffing requires an organization API key.

### Filtering batches by agent

- In the dashboard, an agent's **Batches** tab lists only that agent's batches.
- Via the API, pass `agent_id` as a query parameter to `GET /v2/batches/list` to return only batches staffed by that agent.

### Jumping to a batch's calls

From any batch surface, use **View call logs** to open Conversations filtered to that batch's calls. The dashboard adds a dismissible batch chip so you can clear the filter and see all conversations again.
