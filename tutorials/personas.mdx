...
- **Call Transfers**: Configure when and where the agent hands the call off to a human (see below)

#### Call Transfers

Open the **Behavior** tab and select **Configure Transfers** to set up human handoffs for the persona. Transfers follow the same draft → promote flow as the rest of the persona, so you can iterate safely.

- **Trigger Prompt**: Plain-language description of when the agent should transfer. Be specific — vague triggers like "if the caller is upset" cause false positives. A good trigger reads like "when the caller explicitly asks to speak to a human agent."
- **Default Transfer Number**: Fallback destination in E.164 format (for example, `+15551234567`). Used when the agent decides to transfer but no transfer-list label matches.
- **Transfer List**: Named destinations the agent can pick from based on conversation context. Each entry pairs a label (like `billing` or `sales`) with a phone number. Use a transfer list when different topics should route to different teams.
- **Cold or Warm Transfer**: Cold transfer hands the caller off immediately to the destination number. Warm transfer dials the live agent first, briefs them on the caller and context, then merges the caller in. Warm transfer is Enterprise-only and defaults to off.

Use the trigger prompt and a transfer list when you want one persona to route to multiple teams. Use only the default number when every transfer should go to the same destination.
