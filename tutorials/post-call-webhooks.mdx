...
- `status` — `"ended"`, `"timeout"`, `"blacklisted"`, `"inactive"`, `"read"`, `"link_clicked"`, `"human_handoff"`, or `"human_handoff_ended"`
...
The `human_handoff` and `human_handoff_ended` status events fire once at the transition into and out of a [human handoff](/tutorials/messaging/overview#human-handoff). While a handoff is active, every message webhook's `metadata` field includes a `human_handoff` object with the reason, source, and actor.
