...
The same segment format is available live during the transferred conversation via the [Post-Transfer Transcript Stream](/api-v1/post/calls-id-transcript-stream) WebSocket. The live stream is enabled separately, with [`stream_post_transfer_transcript: true`](/api-v1/post/calls#param-stream-post-transfer-transcript) on an outbound call or [`stream_post_transfer_transcript`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) on an inbound number; it does not require the webhook subscription, and the webhook subscription does not require the stream.
