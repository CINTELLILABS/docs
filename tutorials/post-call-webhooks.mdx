...
<Note>
On redaction-protected calls, cited utterances keep their ids, timings, and confidence scores, but `speaker_name` and `speaker_description` are withheld because they are written from the recording. Use `speaker_id` to distinguish speakers.
</Note>
