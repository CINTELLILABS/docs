...
- For transferred calls opted in with [`include_post_transfer_transcript`](/api-v1/post/calls#param-include-post-transfer-transcript), also includes `post_transfer_transcript` (the conversation after the handoff, with the transferred-to human labeled `representative`) and `transfer_offset_seconds` marking the transfer boundary. The same fields appear on the standalone `citations` webhook event when the delayed post-call webhook is not enabled.
