...
For inbound calls, subscribe on the number instead: set `webhook`, include `"post_transfer_transcript"` in `webhook_events`, and set `record: true` with [Update Inbound Number Details](/api-v1/post/inbound-number-update).
...
The same segment format is available live during the transferred conversation via the [Post-Transfer Transcript Stream](/api-v1/post/calls-id-transcript-stream) WebSocket. The live stream is enabled separately: with [`stream_post_transfer_transcript: true`](/api-v1/post/calls#param-stream-post-transfer-transcript) on Send Call for outbound calls, or with [`stream_post_transfer_transcript: true`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) on the number for inbound calls. It does not require the webhook subscription, and the webhook subscription does not require the stream. On calls with redaction enabled, only the webhook transcript is available, redacted; the live stream is refused.
