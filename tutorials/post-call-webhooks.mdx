...
`human_handoff` fires once when a human takes over a thread (via the API or the **Transfer to Human** pathway node) and `human_handoff_ended` when the agent gets it back. While a handoff is active, message webhooks keep firing for every inbound message, `metadata.human_handoff` carries the reason, source, and any metadata passed at handoff, and no per-message status webhooks are sent.
