...
The `citations` event is used as the delivery channel regardless of contents. You receive it once `include_post_transfer_transcript` is on, even if you have no citation schemas configured and `"citations"` is not in `webhook_events`. When only the transcript applies, the event carries just `post_transfer_transcript` and `transfer_offset_seconds` and no `citations` array. Branch on which properties are present rather than on `event_type`.
