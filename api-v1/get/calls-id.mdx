...
<ResponseField name="citations_status" type="string">
  Indicates the state of the `citations` array so you can tell a genuinely empty result apart from a partial or failed fetch. Only returned for enterprise calls that had a citation schema attached; omitted otherwise.

  - `ok` — citations were fetched successfully. An empty `citations` array means the transcript produced no matches.
  - `incomplete` — citations were returned, but some may be missing due to a partial hydration. Treat the array as a subset of the true result.
  - `unavailable` — citations could not be fetched for this call right now. The `citations` array will be empty. Retry the request later to get the full result.
</ResponseField>
