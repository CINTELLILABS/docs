...
<ParamField query="v" type="string">
  Set to `2` to return `pathway_logs` in the current pathway events format. Without `v=2`, the legacy pathway log may be returned as an empty array with `pathway_logs_message` when the legacy format is unavailable.
</ParamField>
...
<ResponseField name="pathway_logs" type="array | null">
  Pathway events or legacy logs for the call. Pass `v=2` to receive the current pathway events format. During the legacy format rollout this may be an empty array and `pathway_logs_message` explains the deprecation.
</ResponseField>
...
<ResponseField name="warm_transfer_calls" type="object">
...
<ResponseField name="c_id" type="string">
  The call ID as stored internally. This is the same identifier as `call_id`.
</ResponseField>

<ResponseField name="redaction" type="array | null">
  Redaction policies applied to the stored call data.
</ResponseField>
...
<ResponseField name="start_time" type="string | null">
  The requested dispatch time, if one was set.
</ResponseField>

<ResponseField name="analysis_schema" type="object | null">
  The schema used to produce `analysis`.
</ResponseField>

<ResponseField name="pathway_tags" type="array | null">
  Tags produced while the pathway ran.
</ResponseField>

<ResponseField name="recording_expiration" type="string | null">
  The timestamp when the stored recording expires.
</ResponseField>

<ResponseField name="disposition_ids" type="array | null">
  Outcome IDs evaluated for the call.
</ResponseField>

<ResponseField name="agent_experiment_id" type="string | null">
  The agent experiment associated with the call.
</ResponseField>

<ResponseField name="agent_version_id" type="string | null">
  The compiled agent version used for the call.
</ResponseField>

<ResponseField name="persona_id" type="string | null">
  The persona used for the call.
</ResponseField>

<ResponseField name="contact_id" type="string | null">
  The contact associated with the call.
</ResponseField>

<ResponseField name="dispositions" type="array">
  Outcome results attached to the call.
</ResponseField>

<ResponseField name="total_transferred_call_duration" type="number | null">
  Total duration of transferred call legs, in seconds.
</ResponseField>

<ResponseField name="live_translation_transcript" type="array | null">
  Live translation transcript entries, when translation was enabled.
</ResponseField>

<ResponseField name="sid" type="string">
  The telephony provider call SID. Present for BYOT calls.
</ResponseField>

<ResponseField name="twilio_account_sid" type="string">
  The Twilio account SID. Present for BYOT calls.
</ResponseField>
...
<ResponseField name="is_canary" type="boolean">
  Whether the call ran through a canary deployment. Returns `true` if the call was routed through a canary deployment, `false` for production, and `null` if the deployment could not be determined. Use this to programmatically distinguish canary traffic from production when ingesting call data.
</ResponseField>
...
Errors use a flat `{ "status": "error", "message": "..." }` body with status `400`, `404`, or `500`.
