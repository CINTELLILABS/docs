...
description: "Retrieve the pathway event timeline for a call, including transcripts, node transitions, webhooks, tools, and variable extraction."
---
...
<ParamField query="v" type="string">
  Set to `2` to receive the pathway events format documented here. Omitting it returns the deprecated legacy log format, which may be an empty array during the rollout. Always pass `v=2` for new integrations.
</ParamField>
...
The array may be empty when the call cannot be attributed to a pathway. Events are capped by the service limit and redaction rules are applied before they are returned. A failure to fetch `v=2` events returns `500`.
