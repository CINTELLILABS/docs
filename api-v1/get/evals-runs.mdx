...
<ParamField query="agent_id" type="string">
  Filter to runs attributed to this voice agent.
</ParamField>

<ParamField query="agent_version_id" type="string">
  Filter to runs attributed to this voice agent version.
</ParamField>
...
<Note>
  An unsupported `status` or `triggered_by` value returns `400` with the error code `invalid_filter`.
</Note>
    ...
    <ResponseField name="summary" type="object or null">
      ...
      <Expandable title="Summary fields">
        ...
        <ResponseField name="analysis" type="object">
          Optional post-run analysis. See [Get Eval Run](/api-v1/get/evals-runs-id) for its fields.
        </ResponseField>
      </Expandable>
    </ResponseField>
