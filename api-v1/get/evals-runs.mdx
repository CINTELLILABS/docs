...
<ParamField query="agent_id" type="string">
  Filter to runs attributed to this voice agent. When set, the response is scoped to that agent; when omitted, runs from every agent (and unattributed runs) are returned.
</ParamField>

<ParamField query="agent_version_id" type="string">
  Filter to runs attributed to a specific voice agent version. Typically combined with `agent_id`.
</ParamField>
...
<ResponseField name="data" type="array of objects">
  ...
  <Expandable title="EvalRun list item fields">
    ...
    <ResponseField name="agent_id" type="string or null">
      The voice agent this run is attributed to, or `null` for org-wide runs that aren't scoped to a specific agent.
    </ResponseField>
    ...
    "data": [
      {
        ...
        "agent_version_id": "8e7d6c5b-4a39-2018-edcb-a98765432109",
        "metadata": {},
