...
<ResponseField name="data.human_handoff_at" type="string|null">
  ISO timestamp set while a human owns the thread. The agent does not reply until the conversation is [resumed](/api-v1/post/sms-conversations-resume). Messages sent by the rep have `sender: "HUMAN"`.
</ResponseField>

<ResponseField name="data.human_handoff_reason" type="string|null">
  Reason supplied at handoff, if any.
</ResponseField>

<ResponseField name="data.human_handoff_metadata" type="object|null">
  `source` (`"api"`, `"node"`, or `"dashboard"`), optional `actor`, plus any metadata passed at handoff.
</ResponseField>
