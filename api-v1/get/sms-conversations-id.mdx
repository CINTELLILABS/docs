...
<ResponseField name="data.messages[].status" type="string">
  Delivery status of the message. Possible values: `"queued"`, `"sent"`, `"delivered"`, `"read"`, `"failed"`, `"undelivered"`. The `"read"` status is available for RCS and WhatsApp channels.
</ResponseField>
