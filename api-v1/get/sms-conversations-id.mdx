...
<ResponseField name="data.curr_pathway_version" type="string|null">
  The pathway version number in use, as a string.
</ResponseField>
...
<ResponseField name="data.effective_channel" type="string|null">
  The resolved delivery channel for this conversation: `"sms"`, `"rcs"`, `"whatsapp"`, or `"imessage"`. Determined based on the input channel and whether the messaging service supports RCS.
</ResponseField>
...
<ResponseField name="data.messages" type="array">
  An array of message objects in the conversation, sorted chronologically. Messages removed with [Delete Messages from Conversation](/api-v1/delete/sms-conversations-message) are omitted.
</ResponseField>

<ResponseField name="data.messages[].id" type="string">
  The message's external ID (the provider message SID, or the `message_sid` passed to `/v1/sms/create`), as a string. Older messages without an external ID fall back to their internal numeric ID as a string. Use these values with [Delete Messages from Conversation](/api-v1/delete/sms-conversations-message).
</ResponseField>
...
<ResponseField name="data.messages[].attachments" type="array|null">
  Media attachments on the message, or `null`.
</ResponseField>

<ResponseField name="data.messages[].app_card" type="object|null">
  The rendered iMessage app card sent with the message, or `null`.
</ResponseField>

<ResponseField name="data.disposition_logs" type="array">
  An array of outcome run results for this conversation, newest first. Each entry represents one outcome that was evaluated when the conversation ended. Runs from agent tests are excluded.
</ResponseField>
...
<ResponseField name="data.disposition_logs[].reference_type" type="string">
  The type of record the outcome ran against; `"SMS"` for conversation runs.
</ResponseField>
...
- `500` `ERROR_GETTING_CONVERSATION`: unexpected server error.
    ...
    "messages": [
      {
        ...
        "to": "+15550001111",
        "sender": "USER",
        "id": "SM8a1c0e2f7b3d4e5f6a7b8c9d0e1f2a3b",
        "status": null,
        "error_code": null,
        "attachments": null,
        "app_card": null
      },
      {
        ...
        "to": "+15550001111",
        "sender": "AGENT",
        "id": "SM9b2d1f3a8c4e5f6a7b8c9d0e1f2a3b4c",
        "status": "delivered",
        "error_code": null,
        "attachments": null,
        "app_card": null
