...
description: "Returns the name, description, nodes, edges, folder, and production version of a single conversational pathway in your account."
---
...
<ResponseField name="published_at" type="string | null">
  ISO 8601 timestamp of the most recent production publish, or `null` if never published.
</ResponseField>
...
<ResponseField name="nodes" type="array of objects">
  Data about all the nodes in the live (production) pathway. Each node is `{ "id", "type", "data", "position" }`. The pathway's global prompt is returned as a separate array item of the form `{ "globalConfig": { "globalPrompt": "..." } }`.
...
<ResponseField name="edges" type="array of objects">
    ...
    <Accordion title="Edge Object Parameters">
        ...
        - `data.label`: label for this edge. This is what the agent will use to decide which path to take.
        - `data.description`: optional longer description of the transition
        </Accordion>
</ResponseField>
...
| Status | Body |
|---|---|
| `400` | `{"status": "error", "message": "Error fetching pathway"}` (also returned for an unknown `pathway_id`) |
| `403` | `{"status": "error", "message": "Unauthorized"}` when the pathway belongs to another account |
