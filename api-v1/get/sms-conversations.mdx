...
<ParamField query="filters" type="object[]" required={false}>
  ...
  - `field`: one of `created_at`, `updated_at`, `user_number`, `agent_number`, `is_active`, `message_count`, `variables`, `current_node_id`, `curr_pathway_id`, `curr_pathway_version`, `timed_out_at`, `citation_variables`, `disposition_logs`
  - `operator`: one of `eq`, `contains`, `startsWith`, `endsWith`, `gt`, `gte`, `lt`, `lte`, `hasSome`, `none`, `isEmpty`, `isNotEmpty`
  - `value`: the filter value

  Use `disposition_logs` to filter conversations by their outcome results, and `citation_variables` to filter by extracted citation values. Both fields support `hasSome` and `none` (matching against an array of values), as well as `isEmpty` and `isNotEmpty` (no value required).
</ParamField>
