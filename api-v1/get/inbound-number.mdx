...
<ResponseField name="fallback_number" type="string">
  If set, all inbound calls are immediately forwarded to this phone number instead of being handled by the agent. Returns `null` if no fallback is configured.
</ResponseField>
