...
<ResponseField name="data.paid" type="boolean">  
  Whether the organization is on a paid plan. Use this together with `kyc_level` to check whether the account can place international outbound calls without adding a payment method or completing verification. Free, unverified accounts (`paid: false` and `kyc_level: 0`) on the shared placement group are blocked from placing international outbound calls. See [Send Call](/api-v1/post/calls) for the error returned.  
</ResponseField>  
