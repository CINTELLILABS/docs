...
<ResponseField name="calls" type="array">
  ...
  For enterprise calls with a citation schema attached, each call object includes a `citations_status` field (`ok`, `incomplete`, or `unavailable`) so you can tell a genuinely empty `citations` array apart from a partial or failed fetch. See [`citations_status`](/api-v1/get/calls-id#param-citations-status) on the call-details page for the full definition.
</ResponseField>
