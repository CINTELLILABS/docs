...
<Note>
This endpoint only validates that `org_id` is a UUID. The list is the same for every organization and does not check your membership.
</Note>
...
<ResponseField name="data.versions[].git_sha" type="string">
  Always an empty string. The build hash is not exposed.
</ResponseField>
...
<ResponseField name="data.versions[].currently_supported" type="boolean">
  Always `true`. Unsupported versions are not returned.
</ResponseField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. An unknown `service` returns `500` with error `ORG_VERSION_LIST_LOOKUP_ERROR` and `message: "Invalid service target. Available: api_server,ws_server"`.
</ResponseField>
