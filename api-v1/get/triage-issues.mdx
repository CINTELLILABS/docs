...
<ParamField query="agent_id" type="string">
  Only return issues linked to this agent. Matches the `agent_id` set on the issue at creation.
</ParamField>

<ParamField query="agent_version_id" type="string">
  Only return issues reported against this agent version. Combine with `agent_id` to narrow down to a specific agent and version.
</ParamField>
