...
<ParamField query="agent_id" type="string">
  Only return issues stored with this `agent_id`.
</ParamField>

<ParamField query="agent_version_id" type="string">
  Only return issues stored with this `agent_version_id`.
</ParamField>

<ParamField query="dispatch_id" type="string">
  Only return issues stored with this `dispatch_id`.
</ParamField>

<ParamField query="skill_id" type="string">
  Only return issues stored with this `skill_id`.
</ParamField>

<ParamField query="has_alert" type="boolean">
  Pass `true` to return only issues with at least one bound alert, or `false` for issues with none.
</ParamField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. Returns 400 with `{ "error": "bad_request" }` if `sort`, `order`, `date_field`, `date_range`, `resource_type`, or `limit` is outside its allowed values, or the query contains an unknown parameter.
</ResponseField>
