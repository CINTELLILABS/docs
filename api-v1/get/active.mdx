...
<ResponseField name="data[].international" type="boolean | null">
  Whether the call is international. May be `null` when the value is not available.
</ResponseField>

<ResponseField name="data[].status" type="string">
  Current call status: `QUEUED`, `IN_PROGRESS`, or `TRANSFERRED`.
</ResponseField>

<ResponseField name="data[].transferred_to" type="string | null">
  The destination number when the call has been transferred.
</ResponseField>
...
An unexpected failure returns `500` with `data: null` and an `INTERNAL_SERVER_ERROR` entry in `errors`.
