...
<ResponseField name="voices" type="array">
  ...
  <Expandable title="Voice fields">
    <ResponseField name="id" type="string">
      UUID of the voice. Pass this as `voice_id` to [Speak](/api-v2/post/tts) or any voice management endpoint.
    </ResponseField>
    ...
    <ResponseField name="is_live" type="boolean">
      Whether the voice can be used for synthesis. Professional voices remain `false` until promoted.
    </ResponseField>
    <ResponseField name="consistency" type="number | null">
      The stored consistency setting. `null` for V3 voices.
    </ResponseField>
    <ResponseField name="expressiveness" type="number | null">
      The stored expressiveness setting. `null` for V3 voices.
    </ResponseField>
    <ResponseField name="voice_meta" type="object | null">
      V2 engine settings. Returned only for V2 voices.
    </ResponseField>
    <ResponseField name="boost_language_consistency" type="boolean">
      V3 cloning mode. Returned only for V3 voices and defaults to `true` when unset.
    </ResponseField>
...
| Code | HTTP | Meaning |
|---|---|---|
| `VOICE_NOT_FOUND` | 400 | A voice referenced while building the list could not be accessed. |
| `UNEXPECTED_ERROR` | 500 | The voices could not be listed. |
