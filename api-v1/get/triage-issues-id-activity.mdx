...
<ResponseField name="data.items" type="array">
  ...
  <Expandable title="Common fields">
    ...
    <ResponseField name="actor" type="object | null">
      Who or what produced the event. `actor.kind` is `user`, `system`, or `agent`. For `user` actors, `actor.id` is the user ID; events created through an API key carry the ID of the user the key belongs to. For agent entries, `actor.label` is normalized to `Norm` even when the underlying record stores `blandcode`.
    </ResponseField>
