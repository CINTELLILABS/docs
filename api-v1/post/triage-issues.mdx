...
<ParamField body="agent_id" type="string">
  ID of the agent the issue is about. Stored on the issue and available as a filter on [List Issues](/api-v1/get/triage-issues).
</ParamField>

<ParamField body="agent_version_id" type="string">
  ID of the specific agent version the issue is about.
</ParamField>

<ParamField body="dispatch_id" type="string">
  ID of the dispatch that surfaced the issue, if any.
</ParamField>

<ParamField body="skill_id" type="string">
  ID of the skill the issue is about, if any.
</ParamField>
...
<ResponseField name="data" type="object">
  ...
  <Expandable title="Issue fields">
    ...
    <ResponseField name="dispatch_name" type="string | null">
      Display name resolved for `dispatch_id`. `null` when unresolved.
    </ResponseField>
    <ResponseField name="skill_id" type="string | null" />
    <ResponseField name="skill_name" type="string | null">
      Display name resolved for `skill_id`. `null` when unresolved.
    </ResponseField>
    ...
    <ResponseField name="author_id" type="string">
      User ID of the caller that created the issue. For API key requests this is the user the key belongs to.
    </ResponseField>
    ...
    <ResponseField name="linked_alert_count" type="number">
      Number of alerts bound to this issue.
    </ResponseField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. Returns 400 with `{ "error": "bad_request" }` if a field fails validation or the body contains an unknown key. Returns 404 if a `resource_links` entry cannot be found in your org.
</ResponseField>

<Note>
  Actor fields (`author_id`, `attached_by_id`, `created_by_id`, `actor.id`) and the per-user view state behind `has_unread_activity` are keyed to the user the API key belongs to, so every request made with the same key acts as that one user.
</Note>
