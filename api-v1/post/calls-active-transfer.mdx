...
Transfers to international numbers are blocked for non-enterprise accounts and return `403 FORBIDDEN`. Enterprise organizations bypass this check.
...
<ParamField body="transfer_number" type="string" required>
  Destination phone number in [E.164](https://en.wikipedia.org/wiki/E.164) format (for example `+12223334444`). Parsed against the US region by default, so 10-digit US numbers also work. A number that cannot be parsed returns `500`.
</ParamField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. On failure `data` is `null` and `errors` contains one entry:

  | Status | `error` | When |
  |---|---|---|
  | `400` | `VALIDATION_ERROR` | `call_id` or `transfer_number` is missing. |
  | `400` | `NOT_FOUND` | The call does not belong to your organization or is not in progress. |
  | `403` | `FORBIDDEN` | International transfer blocked for your account type. |
  | `500` | `INTERNAL_SERVER_ERROR` | The transfer failed (`"Error transferring call"`). |
</ResponseField>
