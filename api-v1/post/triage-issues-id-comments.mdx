...
<ResponseField name="errors" type="null | array">
  `null` on success. Returns 404 if the issue does not exist. Returns 400 with `{ "error": "bad_request" }` if `detail` is empty or longer than 4000 characters.
</ResponseField>

<Note>
  `actor.id` is the user the API key belongs to. Every comment posted with the same key is attributed to that one user.
</Note>
