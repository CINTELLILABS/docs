...
The endpoint returns HTTP `201` when it creates a contact and `200` when it finds an existing contact.
...
<ResponseField name="data.contact.identifiers" type="array">
  Contact identifiers, including phone numbers, email addresses, and external IDs.
</ResponseField>

<ResponseField name="data.contact.memories" type="array">
  Contact memory records. Each record includes its associated `entities` array.
</ResponseField>

<ResponseField name="data.contact.crm_syncs" type="array">
  CRM synchronization records associated with the contact.
</ResponseField>
