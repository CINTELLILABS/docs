---
title: "Enable or Disable Memory"
api: "POST https://api.bland.ai/v1/memory/enable"
description: "Enable or disable memory for a pathway or persona version."
---

<Note>
  The contact-based Memory API is in beta. This router allows 100 requests per 15 minutes. Further requests return HTTP `429`. Your organization role must be admin, owner, or operator.
</Note>

### Headers

<ParamField header="authorization" type="string" required>
  Your API key for authentication.
</ParamField>

Your organization role must be admin, owner, or operator.

### Body Parameters

Provide **either** pathway **or** persona identifiers (not both).

<ParamField body="pathway_id" type="string">
  Pathway ID. Use with `version_number` and `memory_enabled`.
</ParamField>

<ParamField body="version_number" type="number">
  Pathway version number. Required when `pathway_id` is provided.
</ParamField>

<ParamField body="persona_id" type="string">
  Persona ID. Use with `version_id` and `memory_enabled`.
</ParamField>

<ParamField body="version_id" type="string">
  Persona version ID (UUID). Required when `persona_id` is provided.
</ParamField>

<ParamField body="memory_enabled" type="boolean" required>
  Set to `true` to enable memory, `false` to disable.
</ParamField>

### Response

<ResponseField name="data" type="object">
  Success response data.
</ResponseField>

<ResponseField name="data.message" type="string">
  Success message.
</ResponseField>

<ResponseField name="errors" type="null">
  Error array (null on success).
</ResponseField>

<ResponseExample>
```json Response (pathway)
{
  "data": {
    "message": "Memory enabled successfully for pathway."
  },
  "errors": null
}
```

```json Response (persona)
{
  "data": {
    "message": "Memory enabled successfully for persona."
  },
  "errors": null
}
```

```json Missing or invalid body
{
  "data": null,
  "errors": [
    {
      "error": "BAD_REQUEST",
      "message": "memory_enabled (boolean) is required."
    }
  ]
}
```

```json Neither or both pathway/persona
{
  "data": null,
  "errors": [
    {
      "error": "BAD_REQUEST",
      "message": "Exactly one of pathway_id or persona_id is required."
    }
  ]
}
```

```json Not found / Forbidden
{
  "data": null,
  "errors": [
    {
      "error": "NOT_FOUND",
      "message": "Pathway not found."
    }
  ]
}
```
</ResponseExample>

### Behavior

When memory is **enabled**, the system automatically refreshes entity schemas in the background. No additional action is required. From that point on, memory is populated automatically on every call or SMS conversation that runs through the pathway or persona version.

When memory is **disabled**, no new memory is written. Existing memory data is preserved.

---

Docs for agents: [llms.txt](/llms.txt)
