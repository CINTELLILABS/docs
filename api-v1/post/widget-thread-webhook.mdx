...
<Tabs>
  <Tab title="MESSAGE">
    ...
    <ParamField body="sender_id" type="string" required>
      UUID identifying the live agent sending the message. Stored on the message as `sender_id` so you can distinguish between agents in the thread. Must be a valid UUID; any other string fails with a `500`.
    </ParamField>

    <ParamField body="created_at" type="string">
      Accepted as an ISO 8601 timestamp but not used. The message timestamp is always the time the request is received.
    </ParamField>
  </Tab>
  <Tab title="END_CONVERSATION">
    ...
    <ParamField body="sender_id" type="string">
      Optional identifier for the agent ending the conversation. Not stored.
    </ParamField>
  </Tab>
</Tabs>
...
<ResponseField name="data" type="object">
  For `MESSAGE` requests, returns the created message object (`sender_type` is always `LIVE_AGENT`). If the thread language is not English, `content` holds the translated text and `original_content` the text you sent. For `END_CONVERSATION` requests, returns the updated thread object with `ended_at` set.
</ResponseField>
...
| 404 | `NOT_FOUND` | Widget or thread not found with the given ID. |
