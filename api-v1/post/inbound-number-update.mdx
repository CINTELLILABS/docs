...
<ParamField body="redaction" type="string[]">
  Enable end-of-call transcript redaction for calls that come in on this number, before any transcript data is written to durable storage (database, Redis, webhooks, exports). Accepts any combination of:

  - `pii` — personally identifiable information (names, emails, phone numbers, addresses)
  - `phi` — protected health information
  - `pci` — payment card information

  The agent still has access to the raw transcript **during** the call so it can complete its task; only the redacted transcript is persisted. Both the agent and user sides of the transcript are redacted together for better accuracy.
