...
<ParamField body="webhook_events" type="array">
  ...
  - `evals` (Eval scores from an attached evals workbench, sent as a separate webhook when the run completes)
  - `post_transfer_transcript` (Adds the post-transfer conversation to the post-call webhook when a call to this number is transferred to a human; see [Post-transfer transcript](/tutorials/post-call-webhooks#post-transfer-transcript). Requires `webhook` and `record: true` on the number; recording is not enabled automatically for inbound calls. In limited rollout: if not enabled for your organization, the subscription has no effect.)
...
<ParamField body="stream_post_transfer_transcript" type="boolean" default="false">
  When `true` and a call to this number is transferred to a human, Bland transcribes the conversation after the transfer live and streams it over the [Post-Transfer Transcript Stream](/api-v1/post/calls-id-transcript-stream) WebSocket. Works for both cold and warm transfers.

  This option controls the live stream only. To receive the post-transfer transcript in the post-call webhook, include `"post_transfer_transcript"` in `webhook_events` (with `webhook` and `record: true` set on the number). The two are independent: you can use either one or both.
