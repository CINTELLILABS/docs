...
<ParamField body="record" type="boolean" default="true">
  Controls whether inbound calls to this number are recorded. New phone numbers have recording enabled by default. Set `record` to `false` to turn recording off for this number. When a call completes, you can access the recording through the `recording_url` field in the call details or your webhook.
</ParamField>
