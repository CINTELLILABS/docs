...
<ParamField body="language_detection_options" type="array">
  An array of language codes that steers multilingual transcription toward the languages you expect on inbound calls.

  When you use a multilingual `language` setting like `fluent`, transcription auto-detects the spoken language on every turn. Set `language_detection_options` to limit detection to the languages you expect. For example, `["en", "es"]`.

  See the [Send Call](/api-v1/post/calls#body-language-detection-options) endpoint for supported codes and behavior.
</ParamField>
