...
<ParamField body="fallback_number" type="string">
  A phone number to forward all inbound calls to instead of handling them with the Bland AI agent. When set, any call to this inbound number is immediately redirected to the `fallback_number` — the agent will not run.

  Useful for temporarily routing calls to a human team or another system without removing the inbound number configuration.

  Set to `null` or an empty string to remove the fallback and restore normal agent behavior.
</ParamField>
