...
<ParamField body="stream_post_transfer_transcript" type="boolean">
  When `true`, calls to this number that are transferred to a human (cold or warm) get a live [post-transfer transcript stream](/api-v1/post/calls-id-transcript-stream), the same as setting `stream_post_transfer_transcript: true` on [Send Call](/api-v1/post/calls#param-stream-post-transfer-transcript). Request a connection URL for a call with [Get Transcript Stream URL](/api-v1/post/calls-id-transcript-stream-token).

  The value must be a boolean, or the strings `"true"` / `"false"`. Anything else returns a 400. Omit the field to leave the setting unchanged.

  This option controls the live stream only. To receive the post-transfer transcript in the post-call webhook instead, set a `webhook` on the number, enable `record`, and include `"post_transfer_transcript"` in `webhook_events`. The two are independent: you can use either one or both.
