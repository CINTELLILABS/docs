...
<ParamField body="resumption_speed" type="number" default={2}>
  ...
  - `3` — **Slow**. The agent waits longer before responding.

  Supersedes the deprecated `interruption_threshold`. Pass `null` to clear this setting.
</ParamField>
...
<ParamField body="resumption_speed" type="number" default={2}>
  Controls how quickly the agent takes the floor after the caller stops talking. `1` fast, `2` medium (default), `3` slow. Supersedes the deprecated `interruption_threshold`. Pass `null` to clear this setting.
</ParamField>
