    ...
    ```json Webhook Node
{
  ...
  <Accordion title="Node Data Object Parameters">
      ...
      - `modelOptions`
        ...
        - `interruptibility`
          — How readily the AI stops speaking when the caller talks over it on this node: `0` block (the AI finishes speaking, same as `block_interruptions: true`), `1` difficult, `2` balanced (default), `3` easy, `4` always interrupt (the AI yields at the next word boundary as soon as it detects the caller's voice). Values outside `0`-`4` return a 400. Useful per node: e.g. difficult on a legal-disclaimer node, always interrupt on a fast Q&A node.
        - `temperature`
