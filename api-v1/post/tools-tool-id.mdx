...
<Note>
Custom Tools v1 is a legacy API maintained for existing customers. Built-in integration tools are updated with the [v2 Tools API](/api-v2/post/tools-tool-id). Custom HTTP tools that call your own URL are still managed through this v1 endpoint: the v2 update endpoint rejects them with `INVALID_TOOL_TYPE`.
</Note>

<Warning>
This is a full replace, not a partial update. The body you send becomes the tool's entire definition. Any field you omit is removed, and `staging_tool` is reset to `null` unless you send it again. Fetch the current definition with [Custom Tool Details](/api-v1/get/tools-tool-id) and send the complete object.
</Warning>
...
<ParamField path="tool_id" type="string" required>
    The ID of the Custom Tool you want to update, with or without the `TL-` prefix. Tools owned by another organization return 404 `TOOL_NOT_FOUND`.
</ParamField>
...
<ParamField body="staging_tool" type="object">
    A draft version of the tool definition. Omitting it clears any existing draft.
</ParamField>

<Note>
Apart from the custom-code `timeout` range, the body is stored as sent. The required flags, the `https://` rule, the `GET`/`POST` list, and the reserved words above are not validated. Keys such as `public` or `agent_id` are saved inside the tool definition on update and do not change the tool's visibility or agent attribution.
</Note>

### Response

<ResponseField name="status" type="string">
    `"success"` when the tool was updated.
</ResponseField>

<ResponseField name="tool_id" type="string">
    The `tool_id` you passed in the path.
</ResponseField>
...
<ResponseField name="errors" type="null">
    `null` on success. On failure, an array of `{ error, message }` objects.
</ResponseField>
