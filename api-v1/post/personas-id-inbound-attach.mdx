...
description: "Attach one or more inbound phone numbers to a persona. The same endpoint handles voice, SMS, and WhatsApp numbers. Channel capability comes from the number's own configuration."
---
...
<Note>
  Attaching a number clears its direct `pathway_id`, `pathway_version`, and `start_node_id`. The number is pinned to the persona's current production version (or its draft if no production version exists). Its prior `persona_settings` are replaced with the builder pathway for this persona when one exists, otherwise they are cleared.
</Note>
