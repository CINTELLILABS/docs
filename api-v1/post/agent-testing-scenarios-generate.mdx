...
<Note>
  Sensitive data such as phone numbers, webhook URLs, and API keys is automatically stripped from the generated `request_data`, including when those fields are nested inside objects or arrays.
</Note>
...
<ResponseField name="request_data" type="object">
  Request data extracted from the call with sensitive fields automatically removed. Values stored in encoded form or nested inside objects and lists are preserved so the generated scenario reproduces the original call's inputs faithfully.
</ResponseField>

<ResponseField name="start_node_id" type="string">
  Returned when `pathway_id` is provided and the call ran through (or was transferred into) that pathway. Identifies the node where the pathway actually began on the original call, so the simulated scenario replays just the relevant segment instead of the whole call.

  If the call never entered the specified pathway, this field is omitted and the scenario falls back to the full call transcript.
</ResponseField>
...
```json Response
{
    ...
    "persona_id": null,
    "start_node_id": "node_booking_intro",
    "request_data": {
