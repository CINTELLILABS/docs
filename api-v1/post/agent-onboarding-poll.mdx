...
<ResponseField name="data.phone_number" type="string | null">
  Present when `status` is `approved`. The number provisioned for this org, or `null` when the owner connected without the Agent Phone Plan.
</ResponseField>
...
<ResponseField name="data.plan.status" type="string">
  Subscription status. `active` when the org is on the Agent Phone Plan, `none` when the owner connected without it.
</ResponseField>
