...
Each request mints a fresh WebSocket URL, so you can start listening at any point during a call. Connect before the URL expires (see `expires_in_seconds`); request a new URL if it lapses.
...
<ResponseField name="data" type="object">
  <Expandable title="data properties">
    ...
    <ResponseField name="expires_in_seconds" type="number">
      Number of seconds the returned URL remains valid for opening a connection. Request a new URL if it expires before you connect.
    </ResponseField>
  </Expandable>
</ResponseField>
...
```json Not Supported in Environment
{
  "status": "error",
  "data": null,
  "errors": [
    {
      "error": "NOT_SUPPORTED_IN_ENVIRONMENT",
      "message": "Live listen is not available in this environment"
    }
  ]
}
```
...
| `NOT_SUPPORTED_IN_ENVIRONMENT` | Live listen is not available in this deployment environment |
| `INTERNAL_SERVER_ERROR` | Server error occurred |

## Notes

- You can start a live listen session at any point while the call is active
- The returned URL is valid for a limited time (`expires_in_seconds`); request a new one if it expires before you connect
