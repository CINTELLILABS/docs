...
<ParamField body="boost_language_consistency" type="boolean" default="true">
  V3 voices only. Controls how the clone is applied during synthesis.

  **Usage:**
  - Pass `"true"` (default) for continuation cloning — mimics the source voice most closely. Best for same-language generation.
  - Pass `"false"` for reference cloning — allows more natural cross-lingual generations with slightly looser voice mimicry.
  - Ignored when `isBTTS_V2` is `"true"`.
</ParamField>
