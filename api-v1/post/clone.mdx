...
Three cloning engines are available, selectable per request:

- **BTTS V3** (default), single 10-second sample, 17+ languages, highest fidelity.
- **BTTS V2**, exactly 1 WAV file, 17+ languages, production-stable previous generation.
- **BTTS V1**, 1-5 WAV files (1-60 seconds each), English only, legacy.
...
<ParamField body="isBTTS" type="boolean">
  Use the V1 (legacy) engine instead of V3. Pass `"true"` as a multipart string.
</ParamField>
...
<Accordion title="V1 (isBTTS=true)">
  - 1 to 5 WAV files.
  - Each file 1-60 seconds, max 10 MB.
...
After the clone is created, V1 voices accept additional training samples via [Add Voice Sample](/api-v1/post/voices-samples) (up to 5 total). V2 and V3 voices are single-sample by design.

Use [List Voice Samples](/api-v1/get/voices-id-samples) to inspect the samples currently attached to a voice.
