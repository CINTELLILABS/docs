...
<ParamField body="pass_threshold_pct" type="number or null">
  Percentage of evaluations that must pass for the run to be marked passing overall, 0-100. Omit this field or set it to `null` to run without a threshold.
</ParamField>
...
<ParamField body="agent_id" type="string">
  UUID of the voice agent this run evaluates. This identifies the voice agent, not an eval agent used as a judge.
</ParamField>

<ParamField body="agent_version_id" type="string">
  UUID of the specific voice agent version this run evaluates.
</ParamField>

<ParamField body="async" type="boolean" default="true">
  Whether judging may use delay-tolerant execution lanes. Set this to `false` only when you need synchronous judging within the background run workflow.
</ParamField>
...
<Note>
  You must supply either a non-empty `attached_agents` array or a `workbench_setup_version_id` (whose pinned roster supplies the agents). If you provide `workbench_setup_id` or `workbench_setup_version_id`, both fields are required together.
</Note>

<Warning>
  A valid billing record must be on file before a run can start. The API returns `402 BILLING_RECORD_REQUIRED` when it cannot find one. It returns `402 BILLING_NOT_ALLOWED` when billing denies the run, including when a standard organization's balance is below -1, an enterprise organization's balance is below -5, or a metered billing account is blocked.
</Warning>

### Response

Returns `202 Accepted` after the run and its agent snapshots are created and the background workflow is started.
