...
<ResponseField name="data" type="object">
  An object confirming the message was accepted for delivery.
</ResponseField>

<ResponseField name="data.status" type="string">
  Always `"processing"`. Delivery happens asynchronously in a background workflow.
</ResponseField>

<ResponseField name="data.message" type="string">
  Confirmation text for the accepted request.
</ResponseField>
...
<ResponseField name="data.message_id" type="string">
  ID of the queued message. Only present when `agent_message` or `content_sid` was supplied; omitted when the agent generates the reply. Use it to track delivery status via [Get Conversation](/api-v1/get/sms-conversations-id).
</ResponseField>
...
- `400` `INVALID_TEMPLATE_USAGE`: `content_sid` was supplied without `channel: "whatsapp"`.
- `400` `INVALID_PERSONA_ID`: `persona_id` is not a UUID.
- `400` `NO_PERSONA_VERSION`: the persona has no production version (or no draft version when `persona_version` is `"draft"`).
- `403` `ENTERPRISE_REQUIRED`: your organization is not on an Enterprise plan.
- `404` `PERSONA_NOT_FOUND`: `persona_id` does not belong to your account.
- `404` `INBOUND_NUMBER_NOT_FOUND`: `agent_number` is not a number on your account.
- `500` `ERROR_SENDING_SMS`: unexpected server error.
...
```json Response
{
  "data": {
    ...
    "message_id": "0f7c2d3e-4a5b-4c6d-8e9f-0a1b2c3d4e5f"
  },
  "errors": null
}
```
