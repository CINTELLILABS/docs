...
<ParamField body="channel" type="string" required={false}>
  ...
  For numbers provisioned for iMessage, the channel is auto-derived from the number's configuration, so you don't need to set this explicitly. See [iMessage](/tutorials/imessage) for details.
</ParamField>
