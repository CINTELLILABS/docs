...
<ParamField body="time_out" type="number" required={false}>
  ...
  Minimum accepted value is 30 seconds. Positive values below 30 are raised to 30. `null` or `0` disables the timeout and is stored as-is. The inactivity timeout only governs the automatic timeout flow after a user goes silent; explicit `/v1/sms/send` calls always attempt delivery regardless of prior conversation inactivity.
</ParamField>
