...
<Info>
  **Availability** - SMS sending is available on Enterprise plans and on the [Agent Phone Plan](/platform/agent-phone-plan). Agent Phone Plan organizations can send SMS to US and Canada destinations only. Contact your Bland representative for Enterprise access.
</Info>
...
<ParamField body="objective" type="string" required={false}>
  A goal for this conversation. When set, automatic replies in the conversation work toward this objective instead of the number's generic texting prompt. Max length `20000` characters.

  The objective belongs to the conversation, not the number, so other threads on the same number are unaffected. A later send with a new `objective` replaces it, a send without one keeps it, and `new_conversation` starts without one. Persona and pathway replies don't use the objective.
</ParamField>
