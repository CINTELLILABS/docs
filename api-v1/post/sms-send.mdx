...
<ParamField body="disposition_ids" type="string[]" required={false}>
  An array of outcome IDs to run when the conversation ends. Overrides any outcomes configured on the SMS number. See [Outcomes](/tutorials/outcomes) for details.
</ParamField>

<ParamField body="citation_schema_ids" type="string[]" required={false}>
  An array of citation schema IDs to extract when the conversation ends. Overrides any citation schemas configured on the SMS number. See [Citations](/enterprise-features/citations) for details.
</ParamField>
