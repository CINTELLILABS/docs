---
title: "Pathway Chat"
api: "POST https://api.bland.ai/v1/pathway/chat/{id}"
...
The message to send to the pathway. Omit it (or send an empty string) on the first request to start the conversation: the pathway runs its start node and returns the agent's opening line without adding a user turn to the history.
</ParamField>

<Note>
Each request that carries a non-empty `message` is billed as one pathway chat message.
</Note>
...
| Status | `error` | When |
|---|---|---|
| `400` | `HANDLE_PATHWAY_CHAT_BAD_REQUEST` | `id` is not a known chat. |
| `422` | `ROUTE_NO_MATCH_NO_FALLBACK` | No edge matched and the current node has no fallback. `ext` is a JSON string with `chat_id`, `pathway_id`, `node_id`, and `evaluated_fields`. |
| `500` | `HANDLE_PATHWAY_CHAT_INTERNAL_SERVER_ERROR` | Unexpected failure. |
