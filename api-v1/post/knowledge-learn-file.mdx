...
PDF files are converted in the background. Bland detects PDF content even when the filename lacks a `.pdf` extension. The upload request returns immediately with status `"PROCESSING"`, and conversion of large documents can take several minutes. Poll [Get Knowledge Base](/api-v1/get/knowledge-id) to track progress through the `processing` object, which reports pages converted, percent complete, and an estimated time remaining. A conversion that fails is retried automatically up to three times. If it still fails, the knowledge base is marked `"FAILED"` with an `error_message`, and failed knowledge bases are never used to answer agent queries.
