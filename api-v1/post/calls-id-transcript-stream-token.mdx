...
<ParamField path="call_id" type="string" required>
  The call whose post-transfer transcript you want to stream. Outbound calls must have been created with `stream_post_transfer_transcript: true`; inbound calls must have arrived on a number with [`stream_post_transfer_transcript: true`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) set.
</ParamField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. `401` when the call belongs to a different organization, `403` when the feature is not enabled for your organization, `404` with `REDACTION_ENABLED` when the call has redaction enabled (the live stream serves raw text, so redacted calls get the post-call transcript only, redacted), `404` with `CALL_NOT_FOUND` when the call does not exist or has no active transcript stream (not opted in, not yet answered, or already ended).
</ResponseField>
