...
<ParamField path="call_id" type="string" required>
  The call whose post-transfer transcript you want to stream. The call must have opted into the stream: an outbound call created with `stream_post_transfer_transcript: true`, or an inbound call to a number with [`stream_post_transfer_transcript` enabled](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript).
</ParamField>
...
<ResponseField name="errors" type="null | array">
  `null` on success. `401` when the call belongs to a different organization, `403` when the feature is not enabled for your organization, `404` with `CALL_NOT_FOUND` when the call does not exist or has no active transcript stream (no `stream_post_transfer_transcript` opt-in, not yet answered, or already ended), `404` with `REDACTION_ENABLED` when the call has redaction enabled. The live stream serves raw text, so it is never available on a redacted call; the post-call webhook still carries the transfer transcript, redacted like the rest of the call.
</ResponseField>
