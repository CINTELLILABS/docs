...
- `400` `MISSING_FILE_ID`, `MISSING_AGENT_NUMBER`, `INVALID_GLOBAL_FORMAT`, `INVALID_COLUMN_MAPPING_FORMAT`: the body is missing a required field or has the wrong shape.
- `403` `ENTERPRISE_REQUIRED`: your organization is not on an Enterprise plan.
- `404` `FILE_NOT_FOUND`: `file_id` does not exist, was deleted, or was not uploaded with `file_type: "batches"`.
- `404` `AGENT_NUMBER_NOT_FOUND`: `global.agent_number` is not a number on your account.
- `500`: the batch workflow could not be started.
