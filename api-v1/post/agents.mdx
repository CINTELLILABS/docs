...
<ParamField body="pathway_id" type="string">
    ...
    Warning: Setting a pathway will set the following fields to `null` / their default value - `prompt`, `first_sentence`, `dynamic_data`, `tools`, `transfer_list`
...
<ParamField body="resumption_speed" type="number" default={2}>
  ...
  - `1` — **Fast**. The agent responds sooner.
  - `2` — **Medium** (default). Balanced pacing; reproduces existing behavior.
  - `3` — **Slow**. The agent waits longer before responding — good for collecting detailed information.

  Supersedes the deprecated `interruption_threshold`.
</ParamField>

<ParamField body="interruption_threshold" type="number" default={500} deprecated>
  Deprecated. Use `resumption_speed` instead.

  Adjusts how patient the AI is when waiting for the user to finish speaking. Lower values respond more quickly, higher values wait longer.
