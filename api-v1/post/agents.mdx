...
<ParamField body="noise_cancellation" type="boolean" default={true}>
  Enables background noise suppression on the caller's audio during web calls. When `true`, ambient noise is filtered out before transcription so the agent hears the caller more clearly.

  Set to `false` if you'd rather pass raw audio through unmodified, for example when testing transcription quality or when callers are in already-quiet environments.
</ParamField>
