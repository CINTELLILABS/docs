...
<ParamField body="from" type="string">
  ...
  If you have a SIP trunk configured for outbound, you can also pass a SIP trunk identity attached to your trunk — such as a DID or a dial-plan extension like `2001` — as the `from` value. Phone numbers and SIP identities are forwarded to the trunk verbatim, so you don't need to format them as a standard E.164 phone number. Numbers are resolved regardless of whether the request or the stored identity uses a `+` prefix, so `+14185060701` and `14185060701` both match the same attached number. See [SIP Integration](/enterprise-features/SIP-integration) for how to attach numbers and DIDs to a trunk.
</ParamField>
