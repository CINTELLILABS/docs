...
<ParamField body="transfer_target_type" type="string" default="phone">
  Specifies how the transfer destination should be treated. Accepts `"phone"` or `"did"`.

  - `"phone"` (default) — The transfer destination is a standard PSTN phone number and is validated as E.164.
  - `"did"` — The transfer destination is a free-form SIP/DID target (such as a DID, extension, or `sip:user@host` URI) routed over your organization's [SIP trunk](/enterprise-features/SIP-integration). E.164 validation and geo checks are skipped.

  Use `"did"` when transferring to a SIP destination that is not a standard phone number.
</ParamField>

<ParamField body="enable_sip_refer" type="boolean" default="false">
  For inbound SIP calls performing a cold transfer, hand the call off using SIP REFER instead of bridging a new dial leg. Only applies to SIP-based calls.
</ParamField>

<ParamField body="transfer_extension" type="string">
  An optional extension that is dialed after the transfer connects. Useful when the destination requires an extension (for example, an IVR option or a direct extension on a PBX).
</ParamField>
