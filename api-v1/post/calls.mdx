...
<ParamField body="interruption_threshold" default="500" type="number">
  Adjusts how patient the AI is when waiting for the user to finish speaking, in milliseconds.

  Lower values mean the AI will respond more quickly, while higher values mean the AI will wait longer before responding.

  <Note>
    The `resumption_speed` field (`1`-`3` level) has been removed. Requests that still include it are accepted but ignored. Use `interruption_threshold` to tune how quickly the AI responds after the caller finishes.
  </Note>
</ParamField>
