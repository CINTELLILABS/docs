...
<ParamField body="dialing_strategy" type="object" optional="true">
  Defines how the outbound caller ID number (`from`) should be selected for the call.

  This field replaces both `local_dialing` and `geospatial_dialing`. It supports the following strategies:

  #### 1. `regional_auto`
  Automatically selects a "from" number that matches the callee's area code (U.S. only).
  <br />
  Requires the [Local Dialing add-on](https://app.bland.ai/dashboard/add-ons).
