...
<ParamField body="dialing_strategy" type="object" optional="true">
  Controls how the caller ID number (`from`) is selected when placing an outbound call.

  Use this field to influence how the system chooses a number that appears local or relevant to the callee, improving pickup rates. There are three supported strategies:

  #### 1. `regional_auto`
  Automatically selects a caller ID number with the same area code as the callee (U.S. only).
  Ideal for improving answer rates through local presence.
  Requires the Local Dialing add-on.
  ...
  #### 2. `custom_pool`
  Selects a number from your own pre-configured pool of phone numbers based on geographic proximity to the callee.
  Designed for organizations that want full control over the caller IDs being used.
  Requires Enterprise access and a pool ID provided by your Bland representative.
