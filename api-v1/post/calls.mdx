...
<ParamField body="voicemail" type="object">
  ...
  <Note>
    The effective action is derived from the whole request, not from `action` alone. Supplying a `message` implies leaving one, so `"action": "hangup"` sent alongside a `message` leaves the message. Sending `amd: true` forces `ignore` regardless of `action`. See [Voicemail](/platform/agent-call-recipes#voicemail) for the full precedence.
  </Note>
