...
<ParamField header="encrypted_key" type="string">
  ...
  Also accepted as the `encrypted-key` header or as an `encrypted_key` field in the request body.
...
<ParamField body="phone_number" type="string" required>
  The phone number to call. Must be a valid phone number in [E.164](https://en.wikipedia.org/wiki/E.164) format. Numbers without a country code are treated as US (+1).

  For calls placed over a SIP trunk, a SIP user-part identity (a DID, extension, or alphanumeric alias) is accepted as-is.
</ParamField>
...
<ParamField body="pathway_version" type="integer | string">
  The version of the pathway to use for the call: a version number, or the string `"production"` or `"staging"`. Defaults to the production version.
</ParamField>

<ParamField body="task" type="string">
  Required unless you pass `pathway_id`, `persona_id`, or `agent_id`. Do not specify if using a pathway or an agent.

  Maximum 30,000 characters (100,000 with the `LONGER_CONTEXT` entitlement).
...
<ParamField body="persona_version" type="string" default="production">
  Which version of the persona to apply: `production` or `draft`.
</ParamField>

<ParamField body="agent_id" type="string">
  The UUID of an agent created in the dashboard. The agent's compiled version drives the call, so `task` is not required. Cannot be combined with `pathway_id` or `persona_id`, and `tools` cannot be passed on agent calls (the agent version's own tools are used).
</ParamField>

<ParamField body="agent_version" type="string" default="production">
  Which version of the agent to run. Requires `agent_id`. Accepts a semantic version such as `"1.2.0"`, `"branch:<name>"`, or one of `dev`, `latest`, `staging`, `production`. Defaults to the production pin when omitted.
</ParamField>

<ParamField body="start_node_id" type="string">
  Start the pathway at a specific node instead of its start node. Requires `pathway_id`.
</ParamField>
...
<ParamField body="language" default="babel-en" type="string">
  ...
  <Accordion title="Expand to view language options">
    ...
    - `fluent` - Fluent (Multilingual)<sup>2</sup>
    - `en` - English
    ...
    <sup>2</sup> The `fluent` mode is a multilingual option that auto-detects the spoken language and switches between languages on the fly, powered by a multilingual transcription model. **Note: For pathway calls and prompts, make sure to prompt your agent to respond in the language they are spoken to.**
  </Accordion>
</ParamField>

<ParamField body="language_detection_period" type="integer">
  How long, in seconds, to run automatic language detection at the start of the call (1 to 100). Use with `language_detection_options`.
</ParamField>

<ParamField body="language_detection_options" type="string[]">
  The languages the caller may speak, as language codes (for example `["en", "es"]`). When language detection runs, the transcription switches to the detected language from this list.
</ParamField>
...
<ParamField body="temperature" type="float">
  ...
  Defaults to the model's own setting when omitted: `0.5` for `enhanced`, `0.3` for `base` and `turbo`.
...
<ParamField body="interruptibility" default="2" type="integer">
  ...
  - `4`: Always. Every caller utterance interrupts; backchannel suppression is disabled.
</ParamField>
...
<ParamField body="timezone" type="string">
  Set the timezone for the call, as a TZ database identifier such as `America/Los_Angeles`. Not set by default.

  Pass `"auto"` to resolve the timezone from the dialed number's area code (US numbers). This is also the timezone used to interpret `dispatch_hours`.
...
<ParamField body="dispatch_hours" type="object">
  Only dispatch the call within a daily time window. If the call is sent outside the window it is scheduled for the next window. Both keys are required.

  - `start` (string): Start of the window, for example `"9:00am"` or `"09:00"`.
  - `end` (string): End of the window, for example `"5:00pm"` or `"17:00"`. An end before the start wraps to the next day.

  Times are interpreted in `timezone` (use `"auto"` to derive it from the dialed number). Accepted formats: `h:mma`, `ha`, `H:mm`, `HH:mm`, `HH:mm:ss`.

  ```json
  {
    "timezone": "auto",
    "dispatch_hours": { "start": "9:00am", "end": "5:00pm" }
  }
  ```
</ParamField>
...
<ParamField body="noise_cancellation" default="true" type="boolean">
...
<ParamField body="voicemail" type="object">
  ...
  - `action` (enum): What the AI should do when it detects a voicemail. The default is `"hangup"`. Available options:
    ...
    - `"leave_sms"`: End the call and send the `sms` message.
    - `"leave_message_and_sms"`: Play the `message`, end the call, and send the `sms` message.
...
<ParamField body="stream_post_transfer_transcript" type="boolean" default="false">
  ...
  The legacy name `include_post_transfer_transcript` is accepted as a synonym.
...
<ParamField body="post_call_evals" type="object">
  Attach an [evals workbench](/tutorials/evals) to this call. When the call completes, Bland automatically runs the workbench's eval configuration against the call. No separate eval-run submission is needed.
  ...
  Attaching evals requires the call to be recorded. If you set `record: false` alongside `post_call_evals`, the request is rejected; otherwise recording is enabled automatically.

  The post-call webhook acknowledges the attachment with `post_call_evals: { workbench_setup_id, workbench_setup_version_id, status: "pending" }`. Eval scores arrive later on a separate `evals` webhook event once the run completes.
...
<ParamField body="analysis_prompt" type="string">
  A custom prompt that guides the post-call analysis. Maximum 5,000 characters.
</ParamField>

<ParamField body="analysis_schema" type="object">
  A JSON object describing the fields to extract from the call after it ends. The result is returned as `analysis` in the call details and post-call webhook. An empty object is ignored.
</ParamField>

<ParamField body="analysis_preset" type="string">
  The ID of a saved analysis preset to run after the call.
</ParamField>

<ParamField body="disposition_ids" type="string[]">
  An array of outcome IDs to run against this call when it ends. See [Outcomes](/tutorials/outcomes).
</ParamField>
...
<ParamField body="retry" type="object">
  ...
  - `voicemail_action` (enum): The action to take when the call goes to voicemail. Options: `hangup`, `leave_message`, `ignore`, `leave_sms`, `leave_message_and_sms`.
...
<ParamField body="redaction" type="string[]">
  Compliance regimes to redact from the stored call data (transcripts, recordings, variables). Any combination of `pii`, `phi`, `pci`, and `medical`. Empty or omitted means no redaction.
...
<ParamField body="memory_id" type="string">
  Attach a [memory store](/api-v1/post/memory-create) to the call so the agent shares context with previous conversations with this contact. Passing a `memory_id` enables memory for the call even when the pathway does not have memory turned on.
</ParamField>
...
<ParamField body="keywords" default="[]" type="string[]">
  ...
  For stronger keyword boosts, you can place a colon then an integer boost factor after the word. The default boost factor is 1. At most 20 keywords, each under 100 characters.
...
<ResponseField name="max_duration_notice" type="object">
  Present only when Bland shortened the call's `max_duration` because the account has not completed a purchase. Contains `requested_max_duration` (integer), `max_duration` (integer, the minutes the call will actually run), `reason` (string code), and `message` (string). Buy credits or turn on auto-recharge to lift the limit.
</ResponseField>
...
<Accordion title="View All Error Codes" icon="warning">
  ...
  <ResponseField name="400 - Invalid Voicemail Action" type="object">
    ...
    {
      "status": "error",
      "message": "Invalid voicemail_action. Options are: hangup, leave_message, ignore, leave_sms, leave_message_and_sms."
    }
    ```
  </ResponseField>
