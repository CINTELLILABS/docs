...
<ParamField body="stream_post_transfer_transcript" type="boolean" default="false">
  ...
  The live stream is not available on calls with redaction enabled. [Get Transcript Stream URL](/api-v1/post/calls-id-transcript-stream-token) returns 404 with `REDACTION_ENABLED` for those calls; the post-call webhook still carries the transfer transcript, redacted like the rest of the call.

  For inbound calls, enable the stream on the number instead with [`stream_post_transfer_transcript`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) on Update Inbound Number Details.
