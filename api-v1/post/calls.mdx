...
<ParamField body="stream_post_transfer_transcript" type="boolean" default="false">
  ...
  The live stream is not available on calls with redaction enabled: it serves raw text, so the token endpoint returns 404 with `REDACTION_ENABLED` instead. Inbound calls can opt in too, by enabling [`stream_post_transfer_transcript`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) on the number.
