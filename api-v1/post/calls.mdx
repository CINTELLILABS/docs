...
<ParamField body="language_detection_options" type="array">
  An array of language codes that steers multilingual transcription toward the languages you expect on the call.

  When you use a multilingual `language` setting like `fluent`, transcription auto-detects the spoken language on every turn. If callers have strong accents, detection can flip the transcript into the wrong language mid-call. Set `language_detection_options` to limit detection to the languages you expect.

  ```json
  {
    "language": "fluent",
    "language_detection_options": ["en", "es"]
  }
  ```

  - Providing a single code keeps transcription in that language for the whole call.
  - Regional variants are normalized to their base language (for example, `en-US` becomes `en`).
  - Codes outside the supported set are ignored. Supported codes: `en`, `es`, `fr`, `de`, `it`, `pt`, `tr`, `nl`, `sv`, `no`, `da`, `fi`, `hi`, `vi`, `ar`, `he`, `ja`, `zh`.
  - If omitted, multilingual transcription auto-detects freely across all languages it supports.
</ParamField>
