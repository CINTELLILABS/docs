...
<ParamField body="interruptibility" default="2" type="integer">
  ...
  - `4`: Always interrupt. The AI treats every caller utterance as an interruption and yields at the next word boundary as soon as it detects the caller's voice. The inverse of `0`.

  Values outside `0`-`4` (including non-integers like `1.5`) are rejected with a 400 error.
</ParamField>
