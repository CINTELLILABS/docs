...
<ParamField body="include_post_transfer_transcript" type="boolean" default="false">
  ...
  Post-call delivery does not require any citations configuration. You receive the transcript on the `citations` event whether or not `citation_schema_ids` is set and whether or not `"citations"` is in `webhook_events`. If your org has `delay_post_call_until_citations` enabled, the transcript is folded into the combined post-call webhook instead.
