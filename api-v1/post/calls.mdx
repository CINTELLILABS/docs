...
<Note>
  You can also save a default voicemail behavior on an agent in the dashboard under **Channels → Calls**: hang up, ignore the voicemail and continue, leave a recorded message, or send an SMS instead. Every outbound call the agent places uses the saved setting. Voicemail parameters passed on an individual call override the agent's saved setting.
</Note>
