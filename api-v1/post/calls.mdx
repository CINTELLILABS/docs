...
<ResponseField name="max_duration_notice" type="object">
  Present only when Bland shortened the call's `max_duration` before queuing it. Free, unverified accounts on the shared placement group have a shorter maximum call length; when the request exceeds that limit, the call is still accepted and this object tells you what happened so the platform is not silently cutting calls off.

  Contains:

  - `requested_max_duration` (number): the `max_duration` you asked for, in minutes.
  - `max_duration` (number): the value the call will actually run to, in minutes. Matches the `max_duration` the call was queued with.
  - `reason` (string): stable machine-readable code. Currently `CALL_UNPAID_MAX_DURATION_CAPPED`.
  - `message` (string): human-readable explanation naming the action that lifts the limit (buy credits or turn on auto-recharge).

  Buying credits or turning on auto-recharge removes the cap on subsequent calls.
  ...
  ```json
  {
    ...
    "batch_id": null,
    "max_duration_notice": {
      "requested_max_duration": 30,
      "max_duration": 5,
      "reason": "CALL_UNPAID_MAX_DURATION_CAPPED",
      "message": "This call was limited to 5 minutes because the account has not completed a purchase. Buy credits or turn on auto-recharge to lift the limit."
    }
  }
  ```
</ResponseField>
...
<Accordion title="View All Error Codes" icon="warning">
  ...
  <ResponseField name="400 - CALL_UNPAID_INTL_BLOCK" type="object">
    International outbound calling is blocked for free, unverified accounts on the shared placement group. Add a payment method or complete account verification to enable international calls. Paid, verified, and enterprise accounts are not affected.

    ```json
    {
      "status": "error",
      "message": "International calling requires a paid or verified account. Add a payment method or complete verification to enable international calls.",
      "errors": [
        {
          "error": "CALL_UNPAID_INTL_BLOCK",
          "message": "International calling requires a paid or verified account. Add a payment method or complete verification to enable international calls."
        }
      ]
    }
    ...
    You can check whether the current account is eligible before sending a call by fetching the organization with [Get Organization](/api-v1/get/orgs) and inspecting the `paid` and `kyc_level` fields.
  </ResponseField>
