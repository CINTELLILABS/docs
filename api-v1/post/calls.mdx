...
<Note>
International calls need a completed purchase: buy at least $5 of credits in the [dashboard](https://app.bland.ai/dashboard/pay) under Billing, or turn on auto-recharge. Numbers on the [Agent Phone Plan](/platform/agent-phone-plan) call the US and Canada only, regardless of your credit balance.
</Note>
...
<ParamField header="authorization" type="string" required>
  Your API key for authentication. Send it as `Bearer YOUR_API_KEY`. A bare `Authorization: YOUR_API_KEY` with no prefix is also accepted, so older examples keep working. `x-api-key` is not read and does not authenticate.

  Get a key from the [dashboard](https://app.bland.ai/dashboard/settings). An autonomous agent with no dashboard session can mint its own with the [Agent onboarding API](/platform/connect-your-agent).
</ParamField>
...
<ParamField body="voice" type="string" default="Karen">
  The voice of the AI agent to use. Accepts any form of voice ID, including custom voice clones and voice presets. If you omit it, the call uses Karen.

  Default voices can be referenced directly by their name instead of an id.

  Usage example: voice: "Karen"

  For Bland's recommended voices, call [List voices](/api-v1/get/voices) and pick voices tagged `"Bland Curated"` whose `service` is `BTTS_V3`.
</ParamField>
...
<ParamField body="agent_id" type="string">
  The ID of an agent to run the call from. When set, Bland compiles the agent's version snapshot server-side and uses it to drive the call, so you don't need to manage a pathway manually.

  Cannot be combined with `pathway_id` or `persona_id`.

  When you use `agent_id`, the `task` parameter is not required, and `tools` are not accepted (the agent's own configuration applies).

  Settings defined on the agent snapshot — including `voice`, `language`, and interruption sensitivity — are applied as defaults. Any of these values you pass explicitly in the request body take precedence.

  Example request body:

  ```json
  {
    "phone_number": "+19759347494",
    "agent_id": "a0f0d4ed-f5f5-4f16-b3f9-22166594d7a7"
  }
  ```
</ParamField>

<ParamField body="agent_version" type="number | string">
  Selects which version of the agent to run. Only used when `agent_id` is set. Defaults to the version pinned to the `production` environment when omitted.

  Accepts:

  - A specific version number (for example, `3`).
  - `"production"` — the version pinned to the agent's production environment (default).
  - `"staging"` — the version pinned to the agent's staging environment.
  - `"latest"` or `"dev"` — the most recently saved version of the agent.
...
<ParamField body="task" type="string" required>
  Note: Do not specify if using a pathway or `agent_id`.
...
<ParamField body="noise_cancellation" default="true" type="boolean">
  ...
  This parameter was previously documented as defaulting to `false`. The server has been applying `true` when the field is omitted, so send it explicitly whenever the behavior matters to you.
</ParamField>
...
<ParamField body="ivr_mode" default="false" type="boolean">
  Set to `true` when the number you are calling answers with a phone menu rather than a person.

  IVR mode changes two things. The agent stops responding on the conversational timing it uses with a human, so it does not talk over a menu's prompts or fill its pauses. And voicemail handling is suppressed: the effective voicemail action becomes `"ignore"` no matter what you passed, because a phone tree has no voicemail to leave a message on.

  Pair it with [`precall_dtmf_sequence`](/api-v1/post/calls#param-precall-dtmf-sequence) to play a fixed sequence of digits before the call starts, or leave the agent to navigate the menu from the `task`.

  <Note>
    `amd` is a legacy alias for this parameter. It sets the same switch, and passing `ivr_mode: true` sets `amd` for you. Prefer `ivr_mode`: the name says what it does.
  </Note>
</ParamField>
...
<ParamField body="voicemail" type="object">
  ...
  - `action` (enum): What the AI should do when it detects a voicemail. The default is `"hangup"`. Available options:
    ...
    - `"leave_message_and_sms"`: Play the `message`, end the call, and send the SMS configured in `sms`. Supply both `message` and `sms`: with no `sms` config this falls back to `leave_message`.
  ...
  - `sms` (object): Optional. Configuration for sending an SMS notification when a voicemail is left. Contains:
    - `message` (string): Required. The body of the SMS message. Keep concise and clear.
    - `from` (string): Optional. The phone number to send the SMS from (must be a number you own and have SMS permissions for).

    The destination is resolved by Bland rather than set on the request. There is no `to` field, and passing one has no effect.
  ...
  <Note>
    The effective action is derived from the whole request, not from `action` alone. Supplying a `message` implies leaving one, so `"action": "hangup"` sent alongside a `message` leaves the message. And [`ivr_mode`](/api-v1/post/calls#param-ivr-mode) forces `ignore` regardless of `action`, since a phone menu has no voicemail. See [Voicemail](/platform/agent-call-recipes#voicemail) for the full precedence.
  </Note>
...
<ParamField body="retry" type="object">
  ...
  - `voicemail_action` (enum): The action to take when the call goes to voicemail. Options: `hangup`, `leave_message`, `leave_message_and_sms`, `ignore`.
...
<Accordion title="View All Error Codes" icon="warning">
  ...
  <ResponseField name="400 - Invalid Voicemail Action" type="object">
    ...
    {
      "status": "error",
      "message": "Invalid voicemail action. Must be one of: hangup, leave_message, leave_message_and_sms, ignore"
    }
    ```
  </ResponseField>
