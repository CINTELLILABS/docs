...
<ParamField body="resumption_speed" type="number" default={2}>
  Controls how quickly the agent takes the floor after the caller stops talking. One of three levels:

  - `1` — **Fast**. The agent responds sooner. Good for quick back-and-forth conversations.
  - `2` — **Medium** (default). Balanced pacing. Reproduces existing behavior, so calls that do not set this field are unchanged.
  - `3` — **Slow**. The agent waits longer before responding. Good for collecting detailed information where callers may pause mid-thought.

  Each level tunes the turn-end window, silence ramp, and hold ceiling together — you do not need to configure them individually. `resumption_speed` supersedes the deprecated `interruption_threshold`.
...
<ParamField body="interruption_threshold" default="500" type="number" deprecated>
  Deprecated. Use `resumption_speed` instead.
  ...
  If `resumption_speed` is set, it takes precedence and this value is ignored.
</ParamField>
