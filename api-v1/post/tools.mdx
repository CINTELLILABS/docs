...
<Note>
Custom Tools v1 is a legacy API maintained for existing customers. Built-in integration tools are created and managed with the [v2 Tools API](/api-v2/post/tools). Custom HTTP tools that call your own URL are still created through this v1 endpoint: `POST /v2/tools` only accepts integration tools and rejects custom tools.
</Note>
...
<ParamField body="public" type="boolean" default="false">
    Whether the tool is visible to other organizations. Stored on the tool record, not inside the tool definition.
</ParamField>

<ParamField body="staging_tool" type="object">
    A draft version of the tool definition, stored alongside the live definition. Omit to leave it empty.
</ParamField>

<ParamField body="agent_id" type="string">
    UUID of a web agent to attribute the tool to. Returns 400 `INVALID_PARAMETER` when it is not a valid UUID. Omit for an organization-wide tool.
</ParamField>

<Note>
Apart from `agent_id` and the custom-code `timeout` range, the body is stored as sent. The required flags, the `https://` rule, the `GET`/`POST` list, and the reserved words above are not validated at create time. A definition that breaks them is saved and fails when the agent tries to use it.
</Note>

### Response

<ResponseField name="status" type="string">
    `"success"` when the tool was created.
</ResponseField>
...
<ResponseField name="data" type="null">
    Always `null` on this endpoint. The result is carried in the top-level fields above.
</ResponseField>
...
```json Invalid agent_id (400)
{
    "data": null,
    "errors": [
        {
            "error": "INVALID_PARAMETER",
            "message": "agent_id: Invalid UUID format"
        }
    ]
}
```
...
```json Server error (500)
{
    ...
    "errors": [
        {
            "error": "CREATE_INTERNAL_SERVER_ERROR",
