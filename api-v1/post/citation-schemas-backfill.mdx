...
Call backfill operations are **asynchronous**: you receive a `workflow_id` immediately and poll the [status endpoint](/api-v1/get/citation-schemas-backfill-status) to check progress. SMS conversation backfill is **synchronous**: results are returned directly in the response.
...
<Warning>
  Backfilling will update citation variables for the specified call(s) and schema(s). Variables are merged with existing data: only the variables defined in the schema being backfilled will be updated. Set `preview: true` to extract without storing anything.
</Warning>
...
<ParamField body="conversation_ids" type="string[]">
  An array of SMS conversation IDs to analyze. Maximum 50 conversations per request (counting `conversation_id` too); larger requests fail with `400 TOO_MANY_CONVERSATIONS`. All schemas will be applied to each conversation. Processed synchronously.
</ParamField>

#### Recording URL

<ParamField body="recording_url" type="string">
  A publicly fetchable URL of a dual-channel WAV recording to analyze synchronously with the inline `schema`. Nothing is stored: the response contains the extracted variables directly. Requires `schema`; `schema_id` and `schema_ids` are ignored on this path. Cannot be combined with call or conversation IDs. Returns `400 INVALID_CHANNELS` for mono recordings.
</ParamField>

#### Options

<ParamField body="candidate_model" type="boolean" default="false">
  When `true`, use the candidate citation extraction model for call backfills.
</ParamField>

<ParamField body="preview" type="boolean" default="false">
  When `true`, extract variables but do not write them to the call or conversation. Use this to test a schema against real data without mutating stored citation variables. Echoed back as `preview` in the response.
</ParamField>

<ParamField body="higher_accuracy_citations" type="boolean" default="false">
  When `true`, calls are extracted one variable at a time instead of in a single batch. Slower and more expensive, but more accurate on long transcripts or large schemas. Applies to calls only.
</ParamField>
...
Returns `202 Accepted` when calls are included (async processing), or `200 OK` when only SMS conversations are included (sync processing). All fields below live under the `data` key of the standard `{ "data": ..., "errors": null }` envelope.
...
<ResponseField name="preview" type="boolean">
  Whether this was a preview run (nothing stored).
</ResponseField>
...
<ResponseField name="conversations" type="array">
  ...
  - `status`: `"success"` (all schemas succeeded), `"partial"` (some schemas failed), `"failed"` (all schemas failed), `"skipped"` (no messages), or `"error"`
...
Poll [Get Backfill Status](/api-v1/get/citation-schemas-backfill-status) with the `workflow_id` until `status` is `COMPLETED`, `FAILED`, `TERMINATED`, or `TIMED_OUT`. The completed response carries the full per-call `result` object.
...
3. **When status is `COMPLETED`**, retrieve results from the `result` field of the status response
...
```json Response (202 Accepted)
{
  "data": {
    ...
    "message": "Citation backfill workflow started for 1 call(s) with 1 schema(s). Use the workflow_id to check status at GET /v1/citation_schemas/backfill/status/backfill:call-123:1760635265843"
  },
  "errors": null
}
```
...
```json Response (202 Accepted)
{
  "data": {
    ...
    "message": "Citation backfill workflow started for 3 call(s) with 2 schema(s). Use the workflow_id to check status at GET /v1/citation_schemas/backfill/status/backfill:batch:671b7623-f9ac-46db-95f1-9b504fe59a06:1760635265843"
  },
  "errors": null
}
```
...
- More than 50 SMS conversations in a single request (`TOO_MANY_CONVERSATIONS`)
- `recording_url` points at a mono recording (`INVALID_CHANNELS`) or an unreadable file (`INVALID_RECORDING`)

### 403 Forbidden

Returned when:
- Account does not have enterprise features enabled (`ENTERPRISE_REQUIRED`)
...
- Batches over 250 calls return `result.summary` plus `failed_call_ids` / `partial_call_ids` only, without per-call detail
...
- Use the `result.calls[].error_summary` field for quick diagnosis of partial failures
