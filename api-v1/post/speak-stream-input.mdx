...
<Warning>
  **Deprecated.** New integrations should use [Synthesize Speech](/api-v2/post/tts).
</Warning>
...
If your account is out of credits, the connection is refused at authentication with an `error` frame using code `INSUFFICIENT_CREDITS`, then closed. Add credits and reconnect. Free-quota preview requests (for example, voice sample generation) are exempt.
