...
When a call that opted into the stream is transferred to a human (cold or warm), Bland transcribes the bridged conversation live. Connect to this WebSocket to receive finalized transcript segments as the caller and the transferred-to representative speak.

Both call directions can opt in: create an outbound call with [`stream_post_transfer_transcript: true`](/api-v1/post/calls#param-stream-post-transfer-transcript), or enable [`stream_post_transfer_transcript`](/api-v1/post/inbound-number-update#param-stream-post-transfer-transcript) on an inbound number so every call to that number is covered.
...
<Note>
  The live stream serves raw, unredacted text, so it is not available on calls with redaction enabled. The token endpoint returns 404 with `REDACTION_ENABLED` for those calls. The post-call webhook still carries the transfer transcript, redacted like the rest of the call.
</Note>
