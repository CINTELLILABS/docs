...
<ParamField body="updates" type="object" required>  
  ...
  - `"preferences"` (object) - Preference settings for the organization:  
    ...
    - `"recording_lifespan_days"` (integer) - Controls whether call recording links play without authentication.
      - `0` always requires authentication. Requests to `GET /v1/recordings/{call_id}` must present an API key or a signed-in dashboard session.
      - `1` to `1825` allows unauthenticated playback for that many days after the call, then requires authentication.
      - `-1` never requires authentication.
