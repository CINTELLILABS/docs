...
<ParamField path="phone_number" type="string" required>
  The E.164 formatted phone number to update settings for. URL-encode the leading `+` as `%2B`. This number must currently be attached to the specified persona.
</ParamField>

### Body Parameters

<ParamField body="persona_settings" type="object | null">
...
<Warning>
  `persona_settings` is optional in the request schema. An empty body clears the stored settings just like `{"persona_settings": null}`.
</Warning>
