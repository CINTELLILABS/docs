...
<ParamField body="judge_config" type="object">
  Updated judge configuration. The object accepts these fields:

  - `model_profile_key` (string): selectable model profile key.
  - `temperature` (number or null): value from 0 to 2.
  - `max_output_tokens` (integer or null): value from 1 to 64000.
  - `context_sources` (array of strings): 1 to 21 unique evidence sources. Supported values are `transcript`, `call_metadata`, `pathway_logs`, `pathway_semantics`, `transfer_context`, `disposition_results`, `citation_variables`, `post_call_webhook`, `tool_logs`, `variables`, `analysis`, `call_metrics`, `call_config`, `agent_config`, `pathway_config`, `persona_config`, `contact_memory`, `call_notes`, `generated_call_log_files`, `full_call_log_data`, and `audio_recording`.
</ParamField>

<ParamField body="levels" type="array of objects">
  Verdict levels for graded mode. Pass an empty array for pass/fail mode. Drafts support up to 5 levels. A publishable graded version requires 2 to 5 levels. Each level object requires:
...
<ResponseField name="data.judge_config" type="object">
...
<ResponseField name="data.judge_config.context_sources" type="array of strings">
...
<ResponseField name="data.levels[].color" type="string | null">
