...
`metric_type` is immutable. If you include it (or `id`, `org_id`, `created_at`, `updated_at`) in the body it is ignored. Create a new alarm to monitor a different metric.
...
<ResponseField name="errors" type="null|array">
  `null` on success. Validation failures return `400` with `INVALID_THRESHOLD`, `INVALID_WEBHOOK_CONFIG`, `INVALID_EMAIL`, or `INVALID_SMS_NUMBER`. An unknown alarm returns `404` with `ALARM_NOT_FOUND`.
</ResponseField>
