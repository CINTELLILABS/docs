...
Every field is optional. An omitted field keeps its current value; `null` clears nullable fields. The body is validated strictly, and unknown fields return `400 INVALID_BODY`.

Changing persona metadata or any version field other than a `kb_ids`-only change archives the current draft and creates a new draft version. A `kb_ids`-only change updates the current draft in place.

<ParamField body="id" type="string">
  Accepted by the shared schema. Do not change the persona ID.
</ParamField>
...
<ParamField body="call_config" type="object">
  Any supported [Send Call](/api-v1/post/calls) property. The commonly used fields below are examples, not the complete schema.
...
<ParamField body="auth_config" type="object | null">
  Authentication configuration for the draft. Invalid configurations return `400 INVALID_AUTH_CONFIG`.
</ParamField>

<ParamField body="skills" type="array | null">
  Persona skills. Each entry requires `id`, `name`, `description`, `prompt`, and `condition`, and can include `extractVars` and `tools`.
</ParamField>
...
<ResponseField name="data" type="object">
  ...
  <Expandable title="persona object">
    ...
    <ResponseField name="current_production_version" type="object">
      Complete production version object, including memory, entity schema, authentication, and skill settings.
    </ResponseField>
    <ResponseField name="current_draft_version" type="object">
      Complete draft version object with updated configuration, including memory, entity schema, authentication, and skill settings.
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="errors" type="array | null">
  `null` on success, or a list of error objects. An unknown persona returns `404 PERSONA_NOT_FOUND`; a persona without a draft returns `400 NO_DRAFT_VERSION`.
</ResponseField>
