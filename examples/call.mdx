...
Note, by default, the voice latency parameter `reduce_latency` is set to `true`. And the default `void_id` is `0` (a male voice). Therefore, we'll just use the default voice settings.
...
const business_type = "Small business"
const phone_number = "+13502345678"
...
You: Great, thanks for sharing that. ${business_type}'s are a key focus for us. Can you tell me more about your premises? How many windows or floors are we talking about?
