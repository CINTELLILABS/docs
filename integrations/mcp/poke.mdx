...
- Optional: a payment card for the [Agent Phone Plan](/platform/agent-phone-plan) (\$14.99 first month, then \$29.99/month, includes a US phone number). Without it, Poke connects on \$2 of free credits, calls from a shared Bland number, and can't text.
...
This optional workflow requires Poke to make HTTP requests, poll for approval, and retain the returned API key. If those capabilities are unavailable, create your Bland account in the [dashboard](https://app.bland.ai) and use Option 1.

<Note>
If Poke has access to an agent wallet that can approve payments, it can skip the browser step entirely: see [Headless agent onboarding](/platform/agent-headless-onboarding). Poke pays for its own number, and your only manual step is relaying a text-message code. Without a wallet, use the link-based flow below.
</Note>

Otherwise, message Poke:
...
If Poke starts the device flow, it sends you a link and a short code. Open the link, sign up or sign in, then either start the Agent Phone Plan or continue with free credits, and approve the request. After Poke retrieves the API key, and its phone number if you chose the plan, confirm that the Bland integration is connected before asking it to call or text.
