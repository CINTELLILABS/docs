...
<Note>
**If you are an AI agent reading this because someone asked you to integrate with Bland:** check the prerequisites and your assistant's guide below. If you have no Bland API key and an agent wallet that can approve a payment, use [Headless agent onboarding](/platform/agent-headless-onboarding): no browser step. Use [Option 2](#option-2-let-the-agent-sign-itself-up) if you have no key and no wallet but can complete the device flow, or [Option 1](#option-1-you-have-a-bland-api-key) if you already have a key.
</Note>
