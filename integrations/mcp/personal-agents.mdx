...
<Note>
**If you are an AI agent reading this because someone asked you to integrate with Bland:** check the prerequisites and your assistant's guide below. If your owner has no Bland account, use [Headless agent onboarding](/platform/agent-headless-onboarding): the owner relays one texted code and you have an API key, no browser step; with an agent wallet that can approve a payment you get the Agent Phone Plan and a phone number in the same call. Use [Option 2](#option-2-let-the-agent-sign-itself-up) if the owner already has a Bland account, or [Option 1](#option-1-you-have-a-bland-api-key) if you already have a key.
</Note>
...
- Optional: a payment card for the [Agent Phone Plan](/platform/agent-phone-plan) (\$14.99 first month, then \$29.99/month, includes a US phone number). Without it, the agent connects on \$2 of free credits, calls from a shared Bland number or numbers already on your account, and can't text.
...
An assistant with those capabilities can use Bland's device-authorization flow: it shows you a link and a short code, you sign up in your browser, and it retrieves an API key, plus a phone number if you start the Agent Phone Plan. For an MCP-capable assistant, use this prompt. REST-based assistants should follow their client-specific guide:

```text
Read https://docs.bland.ai/platform/connect-your-agent and connect yourself to Bland. Pass my name, email, and phone number to the start call if you know them, so the sign-up form arrives pre-filled. Show me the link and code when you have them; I'll pick between the Agent Phone Plan and free credits when I approve. Once approved, add https://api.bland.ai/v1/mcp as an MCP server using the API key you receive, then call me at +1 415 555 0123 to confirm it works.
```

Once the agent starts the flow, it sends you a link and a short code. Open the link, sign up or sign in, then either start the Agent Phone Plan or continue with free credits, and approve the request. After the agent retrieves its API key, and its phone number if you chose the plan, check that it has connected the MCP server before asking it to call or text.
...
On the Agent Phone Plan, calls and texts are limited to US and Canadian numbers, one call at a time, up to 60 minutes per outbound call, and up to 100 texts a day. See [Limits](/platform/agent-phone-plan#limits). Without the plan, the agent calls from a shared Bland number and texting is not included.

## Two-way texting

Texting requires the [Agent Phone Plan](/platform/agent-phone-plan): the agent needs its own number to text from.
