---
title: "Bland plugin"
description: "The recommended way to connect Claude Code, Claude Desktop, Cursor, or Codex: build, test, and ship Bland voice agents from your coding agent."
---

The Bland plugin is the recommended way to connect a coding agent to Bland. It layers a full agent-engineering workflow on top of the [Bland MCP server](/integrations/mcp/overview): skills for every part of the platform, `/bland:*` slash commands for a file-based pathway workspace, and Norm, the agent builder with a self-driving convergence loop.
...
Using VS Code, Windsurf, Gemini CLI, or a client the plugin does not support? Connect the server directly from the [MCP clients](/integrations/mcp/clients) page.
...
| knowledge | Knowledge base items the agent retrieves from and cites mid-call. |
