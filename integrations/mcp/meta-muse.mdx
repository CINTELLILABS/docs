...
- Optional: a payment card for the [Agent Phone Plan](/platform/agent-phone-plan) (\$14.99 first month, then \$29.99/month, includes a US phone number). Without it, Muse connects on \$2 of free credits, calls from a shared Bland number, and can't text.
...
This optional workflow requires Muse to make HTTP requests, poll for approval, and save the returned key as a Bland credential. If those capabilities are unavailable, create your Bland account in the [dashboard](https://app.bland.ai) and use Option 1.

<Note>
Some assistants can skip the browser step entirely by paying with their own agent wallet. Muse doesn't have a payment credential it can present on your behalf, so this link-based flow is the only self-setup path available to it.
</Note>

Otherwise, tell Muse:

```text
Connect yourself to Bland using https://api.bland.ai/v1/agent/onboarding/muse. Reuse my existing connection if I have one. Otherwise help me sign up for the Agent Phone Plan, prefilling details you already know. Ask me to approve the current price before purchasing. Store the resulting credential securely and tell me my phone number when setup completes.
```

`https://api.bland.ai/v1/agent/onboarding/muse` is a public setup URL Muse reads once to discover the available signup paths, request formats, and instructions for the current environment. It creates no account, sends no SMS, and takes no payment on its own — those still happen through the signup flow Muse picks.
