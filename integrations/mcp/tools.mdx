...
The Bland MCP server currently exposes **40 tools** plus 3 prompts. All of them are scoped to the organization that owns the API key used to connect.
...
| `wait_for_call` | Read | Wait for a call to finish, then return its call log. Holds the request open for up to `timeoutSeconds` (default 30, max 45) and returns as soon as the call reaches a terminal state. Timing out is not an error: you get the call log as it stands, and calling again keeps waiting. |
...
<Tip>
For the full pathway build-test-commit workflow (local file workspace, simulation, autonomous convergence loop), use the [Bland plugin](/integrations/mcp/norm).
</Tip>
...
Dedicated tools expose a deliberate subset of their endpoint's fields. If a field is missing from a dedicated tool (for example, `create_call` omits voicemail handling, `keywords`, webhooks, retry, recording, and max duration), the capability still exists: look it up in the docs tools, then reach it through the passthrough pair, which covers the entire [Bland REST API](/api-v1/post/calls). The server states this routing rule in its MCP `instructions`, so connected agents learn it at initialization.
