...
The Bland MCP server exposes the tools below plus 3 prompts. All of them are scoped to the organization that owns the API key used to connect.
...
| --- | --- | --- |
| `create_call` | Destructive | Place an outbound call to a phone number, optionally with a task or pathway. Omit `voice` to use Karen. |
| `get_call_log` | Read | Fetch a single call: its transcript, recording, summary, and metadata. |
| `wait_for_call` | Read | Wait up to 45 seconds for a call to finish, then return its call log. Use it instead of polling `get_call_log`. |
| `stop_call` | Destructive | End a call in progress, or cancel one that is still queued. |
| `list_voices` | Read | List Bland's curated voices, the recommended starting point for every voice choice (calls, personas, inbound numbers, pathways). Each result includes the voice's `service` so the generation is visible. Any voice the user names still works. |
