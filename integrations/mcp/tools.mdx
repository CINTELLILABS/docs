...
The Bland MCP server currently exposes **41 tools** plus 3 prompts. All of them are scoped to the organization that owns the API key used to connect.
...
| `stop_call` | Destructive | End a call in progress or cancel a queued call by its call id. |
...
| Tool | Access | Description |
| --- | --- | --- |
| `list_voices` | Read | List Bland's curated voices so an agent can pick one to pass as `voice` when it places a call. |
