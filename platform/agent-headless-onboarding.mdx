...
<Steps>
  <Step title="Start the flow">
    ...
    You can also declare how your agent expects to pay at `/complete` with an optional `payment_intent` field, so a flow that can't finish is refused before a text is sent:

    - `"shared_payment_token"`: your agent will present a token from an agent wallet. Same behavior as omitting the field.
    - `"none"`: your agent has no way to pay. If free signup is on, `/start` proceeds and a text is sent as usual. If free signup is off, `/start` answers `402 PLAN_REQUIRED` right away with the link-based `fallback` bundled, without sending a code or spending your per-phone send budget.
...
<Note>
`/complete` accepts a shared payment token or an `Authorization: Payment` credential from the [Machine Payments Protocol](#or-let-the-402-drive-the-payment) round trip. It doesn't accept card numbers, including virtual cards that a Link wallet might mint by default. If your agent has a Link tool that returns a card, look for the shared-payment-token action instead, or drive the flow with the MPP challenge below.
</Note>
...
`/complete` also speaks the [Machine Payments Protocol](https://mpp.dev). To ask for the challenge explicitly, send `payment: { "request_challenge": true }`: `/complete` answers `402 PLAN_REQUIRED` with the challenge whether or not free signup is on, and the session stays verified so a retry with the credential skips the code check. Without `payment`, the same call answers `402 PLAN_REQUIRED` with the challenge only when free signup is off; while free signup is on (it is today), it creates a free account instead.

Sending `request_challenge: true` together with a `shared_payment_token` is refused as `400 INVALID_BODY`. Pick one: the body token to pay, or the challenge to negotiate one.
...
The first call verifies the code and returns the challenge, so the retry with the credential skips the code check. The `200` carries a `Payment-Receipt` header naming the settled payment. A credential that doesn't verify (expired, or not issued by this server) answers `402 PAYMENT_CREDENTIAL_INVALID` with a fresh challenge; repeat the request without the credential to get a new one. If both a body `shared_payment_token` and an `Authorization: Payment` credential are present, the body wins.
...
If your agent omits `payment`, `/complete` still verifies the code and answers `200` with `phone_number: null`, `plan.status: "none"`, and `provisioning: "ready"`. The account starts with free credits, calls from a shared Bland number, can't text, and can't call internationally until the owner completes a purchase. `next_steps` says how to change each of those, including a line naming the two ways the owner adds the Agent Phone Plan to get a dedicated number: the link flow driven from your agent, or the [Billing & Plans](https://app.bland.ai/dashboard/pay?tab=plan) page in the dashboard (see [Adding the plan later](#adding-the-plan-later)).
...
`fallback` carries everything [`POST /v1/agent/onboarding/start`](/platform/connect-your-agent) returns except `verification_url`. Show `verification_url_complete` (or `user_code`) to the owner so they can open it in their own browser, then poll [`POST /v1/agent/onboarding/poll`](/platform/connect-your-agent) with `device_code` until they approve. The link isn't for your agent to open in a cloud-computer browser: it's for the human owner, and the sign-in page will block a bot. `fallback` also rides along on `402 PLAN_REQUIRED` and `409 ACCOUNT_EXISTS`.
...
| `PLAN_REQUIRED` | 402 | You sent `payment_intent: "none"` and free signup is off, so the flow can't finish. Plus fallback; no code was sent. Use the [link-based flow](/platform/connect-your-agent) instead. |
...
| `PLAN_REQUIRED` | 402 | The request asked for a payment challenge (`payment.request_challenge: true`), or no `payment` was included and free signup is off. Plus fallback, and a `WWW-Authenticate: Payment` challenge (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). The session is still usable. |
