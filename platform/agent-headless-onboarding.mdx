...
<Steps>
  <Step title="Start the flow">
    ...
    One phone number can receive at most 3 verification codes per 15 minutes. A fourth `/start` for the same number inside that window is refused with `429 TOO_MANY_REQUESTS` and a `Retry-After` header; no text is sent and no session is created. Other numbers are unaffected.
  </Step>
...
## Pay with a wallet credential

If your agent's wallet speaks the [Machine Payments Protocol](https://www.rfc-editor.org/rfc/rfc9110) (for example, Stripe's `link-cli mpp pay`), it can pay from the 402 alone, with no prior knowledge of Bland's plan or price.

Call `/headless/complete` with a valid session and OTP but no `payment` in the body. The response is `402 PLAN_REQUIRED` with a `WWW-Authenticate: Payment` header that names Bland's Stripe seller profile, the plan's exact price, and an expiry that matches your session's:

```http 402 PLAN_REQUIRED
HTTP/1.1 402 Payment Required
WWW-Authenticate: Payment realm="api.bland.ai", ...
```

Hand that header to your wallet. It returns an `Authorization: Payment` credential that authorizes the exact charge. Retry `/headless/complete` with the credential in the header instead of a token in the body:
...
A valid credential charges through the same path as `payment.shared_payment_token` and returns the same 200 payload. The response also carries a `Payment-Receipt` header referencing the settled charge.

If both a header credential and `payment.shared_payment_token` are present, the body wins.

A credential that fails signature, expiry, or price validation is answered as `402 PAYMENT_CREDENTIAL_INVALID` with a fresh challenge and the device-flow `fallback` block. The session is not spent, so your wallet can retry against the new challenge.
...
| `TOO_MANY_REQUESTS` | 429 | Rate limited. Respect the `Retry-After` header and retry. `/start` allows at most 3 verification codes to one phone number per 15 minutes; other numbers are unaffected. |
...
| `PLAN_REQUIRED` | 402 | No `payment` was included. Plus fallback. The response also carries a `WWW-Authenticate: Payment` challenge when the Machine Payments Protocol is enabled. |
| `PAYMENT_CREDENTIAL_INVALID` | 402 | An `Authorization: Payment` credential was presented but failed signature, expiry, or price validation. Plus a fresh challenge and fallback. The session is still usable. |
