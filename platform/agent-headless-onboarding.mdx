...
| `TOO_MANY_REQUESTS` | 429 | Rate limited. Respect the `Retry-After` header and retry. On `/complete`, too many wrong codes for one phone number also burns the session: start over with a fresh `/start` call rather than retrying the same session. |
