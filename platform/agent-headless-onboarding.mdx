...
- An agent wallet that can approve a single-use shared payment token (`spt_`) scoped to a Stripe business profile and an exact amount, or answer a Machine Payments Protocol challenge. The Link CLI does both; a Link virtual card does not work here. See [Paying with an agent wallet](#paying-with-an-agent-wallet).
...
<Steps>
  <Step title="Start the flow">
    ...
    `client_name` is optional (up to 64 characters, letters, numbers, spaces, and `._/-`). Send the name of the platform you run on (`grok`, `openclaw`, `poke`, `meta`, or your product's name): it never reaches the owner and lets Bland see which assistants complete setup.

    `payment_intent` is optional: `"shared_payment_token"` if you will pay with an agent wallet, `"none"` if you won't. With `"none"`, and only when free signup is off, `/start` answers `402 PLAN_REQUIRED` with the browser `fallback` before any text is sent, so the owner isn't asked for a code the flow can't use. Today free signup is on, so `"none"` proceeds like any other start. `owner_phone` must be E.164 (for example `+14155551234`). `owner_email` must be a valid email address.
  ...
  <Step title="Complete the flow">
    ...
    `payment.shared_payment_token` is the single-use token your agent wallet issued for this charge. The other accepted credential is an `Authorization: Payment` header built from the challenge (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). Card details, including a Link virtual card, are refused. Either credential settles the first month of the Agent Phone Plan (\$14.99 first month, then \$29.99/month).

    The session lasts 10 minutes from `/start`. The first successful code check, whether from a `request_challenge` call or a paying call, restarts that window once. A re-issued challenge doesn't restart it again, so the wallet approval has to complete inside it. If the session expires, start over with a new `/start` call and a new code.
...
The Agent Phone Plan is \$14.99 for your first month (50% off), then \$29.99/month. The first-month price applies to an owner's first plan; a repeat buyer's challenge names `2999`. The token you present must be scoped to the seller and the amount named in the payment challenge (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). Read them from the challenge rather than from this page: the price comes from the plan catalog and can change. At the time of writing the challenge names:

| Field | Value |
| --- | --- |
| Seller (Stripe business profile, the wallet's `network_id`) | `profile_61VOTHaCdbmfq6PD5A6VOTHa3XSQ3zOF0yDybvQ6S1k8` |
| Amount | `1499` (cents, `usd`). The amount comes from the payment challenge and changes when the offer does. |
...
Prefer `link-cli mpp pay`, which reads the challenge and retries `/complete` for you (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). Mint the token yourself when you need to control the spend request.

### Mint the token with Stripe Link

With the Link CLI (`link-cli auth login` once, approved by the owner in the Link app), decode the challenge to read the seller and the amount:

```bash
npx @stripe/link-cli mpp decode --challenge 'Payment id="...", realm="api.bland.ai", method="stripe", intent="charge", request="eyJhbW91bnQiOiIyOTk5Ii...", expires="2026-09-13T06:12:59.687Z"'
```
...
npx @stripe/link-cli spend-request create \
  ...
  --amount 1499 \
  --context "First month of the Bland Agent Phone Plan for <owner>: a dedicated US phone number the agent calls and texts from, billed at 14.99 USD for the first month, then 29.99 USD monthly." \
  --request-approval
```
...
`/complete` also speaks the [Machine Payments Protocol](https://mpp.dev). Send `payment: {"request_challenge": true}` with the session and the code, and the response is `402 PLAN_REQUIRED` with a challenge instead of a free account:
...
| `PAYMENT_METHOD_UNSUPPORTED` | 400 | The body carried `payment.stripe_payment_method`. Card details, including a Link virtual card, can't pass through this endpoint. Send `payment.shared_payment_token` or answer the payment challenge instead. |
| `PLAN_REQUIRED` | 402 | The code verified but no accepted payment arrived: `payment.request_challenge` was sent, or no `payment` while free signup is off. The message names the two ways to pay: `payment.shared_payment_token` (`spt_`), or the `WWW-Authenticate: Payment` challenge on this response answered with `link-cli mpp pay` (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). A Link virtual card is not accepted. Plus fallback, for the owner's own browser. The session is still usable. On `/start`, only with `payment_intent: "none"` while free signup is off; plus fallback, and no text is sent. |
