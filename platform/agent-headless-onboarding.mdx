...
`request` is base64url JSON with the amount, the currency, and the seller profile under `methodDetails.networkId`. A wallet that speaks the protocol reads the challenge, mints the token, and repeats the same request with `Authorization: Payment <credential>`; the credential wins over `request_challenge` on the retry. Stripe's wallet CLI does the whole round trip:
...
| `PLAN_REQUIRED` | 402 | `payment.request_challenge` was sent, or no `payment` while free signup is off. Plus fallback, and a `WWW-Authenticate: Payment` challenge (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). The session is still usable. On `/start`, only with `payment_intent: "none"` while free signup is off; plus fallback, and no text is sent. |
