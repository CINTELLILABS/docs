...
The Agent Phone Plan is \$2.99 for your first month, then \$29.99/month. Offer ends September 17, 2026, 11:59 PM Pacific. The token you present must be scoped to the seller and the amount named in the payment challenge (see [Or let the 402 drive the payment](#or-let-the-402-drive-the-payment)). Read them from the challenge rather than from this page: the price comes from the plan catalog and can change. At the time of writing the challenge names:

| Field | Value |
| --- | --- |
| Seller (Stripe business profile, the wallet's `network_id`) | `profile_61VOTHaCdbmfq6PD5A6VOTHa3XSQ3zOF0yDybvQ6S1k8` |
| Amount | `299` (cents, `usd`). The amount comes from the payment challenge and returns to `2999` after the offer ends. |
...
npx @stripe/link-cli spend-request create \
  ...
  --amount 299 \
  --context "First month of the Bland Agent Phone Plan for <owner>: a dedicated US phone number the agent calls and texts from, billed at 2.99 USD for the first month, then 29.99 USD monthly." \
  --request-approval
```
