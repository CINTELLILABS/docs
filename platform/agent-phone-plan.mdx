...
Outbound calls placed without a `from` number leave from the plan's number rather than a shared pool, so the person you called can call or text it back. Passing an explicit `from` keeps it.
