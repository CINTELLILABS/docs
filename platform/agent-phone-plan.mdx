...
<Tabs>
  ...
  <Tab title="From your agent">
    If your agent has an agent wallet (for example Stripe Link), it can add the plan to an existing organization without a browser. The owner approves a single-use payment token in the wallet, and your agent calls [`POST /v1/agent/phone_plan`](/platform/agent-headless-onboarding#buying-the-plan-for-an-existing-org) or the [`buy_phone_plan` MCP tool](/platform/agent-headless-onboarding#the-buy-phone-plan-mcp-tool) with the token. The response returns the plan and the dedicated number in the same call. Only an owner or admin of the org can buy the plan this way.
  </Tab>
</Tabs>
