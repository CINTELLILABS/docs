...
description: "The device-authorization flow an AI agent follows to sign itself up for Bland: it shows its owner a link and a code, the owner finishes signup in a browser, and the agent gets an API key and a phone number."
---
...
<Note>
By default this flow signs the owner up for the [Agent Phone Plan](/platform/agent-phone-plan). The owner can instead choose to connect without the plan: the agent runs on the org's free credits, calls leave from a shared Bland number, and no dedicated number or texting is included. If the owner already has a Bland account, they sign in instead of creating a new one.
</Note>
...
<Steps>
  <Step title="Start the flow">
    ...
    You can also pass a `prefill` object (`name` or `first_name`/`last_name`, `email`, `phone`) with details you already know about your owner. Accepted fields ride in the URL fragment of `verification_url_complete` and pre-fill the sign-up form. They are never stored, and invalid fields are dropped rather than failing the request. See [Start Agent Onboarding](/api-v1/post/agent-onboarding-start) for the field rules.
  </Step>
  ...
  <Step title="Show the link and code to your owner">
    ...
    The owner opens the link, signs up or logs in, enters the code if it isn't already pre-filled, and either subscribes to the [Agent Phone Plan](/platform/agent-phone-plan) or connects without it on the org's free credits. This happens once, in their browser. Your bot doesn't participate in it beyond polling for the result.
  </Step>
  ...
  <Step title="Poll until approved">
    ...
    | `approved` | Onboarding is complete. The response includes `api_key`, `org_id`, and `phone_number` (`null` if the owner connected without the plan). |
  ...
  <Step title="Use the key">
    ...
    If the owner connected without the plan, `phone_number` is `null` and `plan.status` is `none`. The key works the same way, calls leave from a shared Bland number, and texting isn't available until the org subscribes.
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="MCP server (stdio)">
    ...
    ```bash
    claude mcp add bland-agent -- npx -y bland-cli@0.9.0 mcp
    ```

    Any other MCP host that launches stdio servers takes the same command:

    ```json
    { "mcpServers": { "bland-agent": { "command": "npx", "args": ["-y", "bland-cli@0.9.0", "mcp"] } } }
    ```
...
Outbound texts go through [`POST /v1/sms/send`](/api-v1/post/sms-send). Replies arrive at the same number and are answered automatically. When a thread has a goal, pass an `objective` on the send: replies in that conversation work toward it instead of the number's generic texting prompt.
...
curl -X POST https://api.bland.ai/v1/sms/send \
  ...
  -d '{
    ...
    "user_number": "+14155559876",
    "agent_message": "Hi! I would like to book a haircut for next Tuesday afternoon. What times do you have?",
    "objective": "You are booking a haircut for the owner. Ask for the next available Tuesday afternoon, confirm the time and price, then end the conversation."
  }'
```

The objective belongs to that one conversation, so parallel threads on the same number can each carry their own. A later send with a new `objective` replaces it, and a send without one keeps it. Before your first text thread, configure SMS on the number once with [Update SMS Configuration](/api-v1/post/sms-update).
...
| `PLAN_REQUIRED` | 402 | `/approve` (browser) | The org doesn't have an active Agent Phone Plan subscription and didn't opt to connect without one. |
