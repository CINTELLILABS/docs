...
description: "The device-authorization flow an AI agent follows to sign itself up for Bland: it shows its owner a link and a code, the owner finishes signup in a browser, and the agent gets an API key, plus a phone number if the owner starts the Agent Phone Plan."
---

## Overview

This page is written for the agent. If you are a person setting up an assistant like Grok Bot, start at [Personal AI Agents](/integrations/mcp/personal-agents) and paste the prompt there; your agent reads this page to carry out the flow.
...
At the end of the flow, the bot receives a dedicated API key. If the owner starts the [Agent Phone Plan](/platform/agent-phone-plan), the bot also receives the phone number provisioned for it. If the owner continues with free credits instead, `phone_number` is `null`. The owner never sees or handles the key.

<Note>
During signup, the owner chooses between the [Agent Phone Plan](/platform/agent-phone-plan) and connecting on the org's \$2 of free credits. If the owner already has a Bland account, they sign in and choose there instead of creating a new one.
</Note>
...
<Steps>
  <Step title="Start the flow">
    ...
    `prefill` is also optional. If you already know your owner's name, email, or phone number, pass them so the sign-up form arrives pre-filled:

    ```bash
    curl -X POST https://api.bland.ai/v1/agent/onboarding/start \
      -H "Content-Type: application/json" \
      -d '{"client_name": "my-bot", "prefill": {"name": "Sam Lee", "email": "sam@example.com", "phone": "+14155550123"}}'
    ```

    `prefill` takes `name`, `first_name`, `last_name`, `email`, and `phone`, each a string up to 128 characters. When `first_name` and `last_name` are absent, `name` is split on its first space. `phone` is normalized to E.164, with US as the default region. `email` must have the shape `x@y.z`. Invalid values are dropped silently; a bad pre-fill never fails the request.

    Values that survive validation are appended to `verification_url_complete` as a URL fragment, form-encoded the way `URLSearchParams` does it (`@` becomes `%40`, `+` becomes `%2B`, a space becomes `+`):

    ```text
    https://app.bland.ai/agent-setup?code=BQPL-7VXR#first_name=Sam&last_name=Lee&email=sam%40example.com&phone=%2B14155550123
    ```

    The sign-up page reads the fragment and pre-fills the form. Pre-fill values ride only in the fragment, which stays in the owner's browser; Bland never stores or logs them. If you build the link yourself from `verification_url` and `user_code`, you can append a fragment in the same format.
  </Step>
  ...
  <Step title="Show the link and code to your owner">
    ...
    The owner opens the link, signs up or logs in, enters the code if it isn't already pre-filled, and then either starts the [Agent Phone Plan](/platform/agent-phone-plan) or connects you on the org's free credits. This happens once, in their browser. Your bot doesn't participate in it beyond polling for the result.
  </Step>
  ...
  <Step title="Poll until approved">
    ...
    | `approved` | Onboarding is complete. The response includes `api_key`, `org_id`, and `phone_number` (`null` when the owner connected you without the plan). |
  ...
  <Step title="Use the key">
    Once `status` is `approved`, the response depends on the choice the owner made:
    ...
    When the owner connected you without the plan, `phone_number` is `null` and `plan.status` is `"none"`. The API key works the same way; your calls go out from Bland's shared pool of numbers, and texting is not included. The response also carries a `next_steps` array naming the two ways the owner adds the Agent Phone Plan to get a dedicated number: rerun the link flow so the plan and number come back through your next poll, or the owner adds it from [Billing & Plans](https://app.bland.ai/dashboard/pay?tab=plan).
...
<Tabs>
  ...
  <Tab title="MCP server (stdio)">
    Run the Bland CLI's MCP server as a stdio server and your agent can run the whole flow itself, with no API key set up first. In Claude Code:

    ```bash
    claude mcp add bland-agent -- npx -y bland-cli@0.10.0 mcp
    ```

    Any other MCP host that launches stdio servers takes the same command:

    ```json
    { "mcpServers": { "bland-agent": { "command": "npx", "args": ["-y", "bland-cli@0.10.0", "mcp"] } } }
    ```

    That server exposes two tools for this flow:
    ...
    The [Bland plugin](/integrations/mcp/norm) for Claude Code, Cursor, and Codex uses the hosted MCP server instead and needs an API key configured up front; use it once your agent has its key.
  </Tab>
</Tabs>

## Texting from the new number

Texting requires the [Agent Phone Plan](/platform/agent-phone-plan). If the owner connected you with free credits, there is no dedicated number to text from, and this section doesn't apply until they add the plan.

Outbound texts go through [`POST /v1/sms/send`](/api-v1/post/sms-send). Replies arrive at the same number and are answered automatically. When a thread has a goal, pass it as `objective`, and every reply in that conversation works toward it:

```bash
curl -X POST https://api.bland.ai/v1/sms/send \
  -H "authorization: $API_KEY" \
  -H "content-type: application/json" \
  -d '{"user_number": "+14155550123", "agent_number": "+12765550123", "agent_message": "Hi, do you have an opening next Tuesday afternoon for a haircut?", "objective": "Book a haircut for the owner next Tuesday afternoon. Confirm the time and price, then end the conversation."}'
```

A conversation without an objective is answered from the number's texting prompt, which starts out generic. To change that default for every conversation on the number, use [Update SMS Configuration](/api-v1/post/sms-update).

Calls use the same number. A call placed without `from` leaves from the plan's number, so the person you called can call or text it back.
...
| `PLAN_REQUIRED` | 402 | `/approve` (browser) | The org has no active Agent Phone Plan and the request didn't opt out with `skip_plan: true`. Returned only on the plan path; connecting with free credits never hits it. |
