...
<Steps>
  ...
  <Step title="Use the key">
    ...
    When the owner connected you without the plan, `phone_number` is `null` and `plan.status` is `"none"`. The API key works the same way; your calls go out from Bland's shared pool of numbers, and texting is not included. The owner can add the plan later from [Billing & Plans](https://app.bland.ai/dashboard/pay?tab=plan).
