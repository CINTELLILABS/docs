...
Pathways and personas both become agents. A pathway's node graph becomes a pathway inside an agent. A persona's skills become simple scenarios. The global prompt that floated beside the graph is now the agent's system prompt, in its settings.
