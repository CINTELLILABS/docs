...
In **Phone numbers**, a number that still answers a pathway from the previous dashboard shows that pathway as its owner, in both the table and the number's detail panel. When you assign one of these numbers to an agent, the dashboard asks you to confirm and names the pathway the number leaves. The new dashboard cannot put the number back on its pathway, so confirm only when the agent is ready to take its calls.
