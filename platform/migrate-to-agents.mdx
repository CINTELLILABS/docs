---
title: "Migrate your existing pathways and personas to an Agent"
description: "A dedicated flow for converting an existing conversational pathway into an agent: drop one prompt into your agent of choice (e.g. Claude Code or Codex) and it will install and run the Bland plugin for you. You simply need to review the outputted agent."
---

Agents are the successor to conversational pathways: the same conversation engine, with a new architecture that places more emphasis on the agent, but still offers the control of pathways using "scenarios", versioned snapshots, environments, and first-class testing. This document will walk you through how to migrate an existing pathway or persona to our agent architecture on your own, giving you maximum control and visibility into how your agent is built.
...
<CardGroup cols={3}>
  <Card title="Migrate with your agent" icon="terminal">
    **This guide.** A self-managed flow where you use your agent of choice to run the migration on your own. The same rigor as a managed migration, but you watch every step, approve the architecture, and can steer. Runs on your own agent subscription.
  </Card>
  <Card title="Migrate with Bland" icon="server">
    Bland runs the full agent-driven migration described below on our infrastructure, and you watch live status. Expect 30–90 minutes for a complex pathway.
  </Card>
  <Card title="Import" icon="bolt">
    **Fastest.** A mechanical 1:1 conversion, good for simple pathways you want to start iterating on as agents.
  </Card>
</CardGroup>

The rest of this page is the **Migrate with your agent** flow, two ways: **Option 1** is one prompt that lets your agent drive the setup; **Option 2** is the step-by-step walkthrough if you'd rather do the setup yourself and understand each piece.
...
- **Move traffic when ready.** Attach or move phone numbers in the dashboard — the migration never touches routing, so your existing pathway keeps answering until you promote the agent to production.
