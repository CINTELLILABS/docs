...
This page covers the move to agents end to end: what changes, where every V1 feature went, the three ways to migrate your existing pathways and personas, and what to do once your agent exists. It goes into the most detail on managing the migration yourself.

Agents are the successor to conversational pathways: the same conversation engine, with a new architecture that places more emphasis on the agent, but still offers the control of pathways using "scenarios", versioned snapshots, environments, and first-class testing. Every path below hands you an agent to review before it takes traffic, so run the simulation suite, test-chat the agent in the dashboard, and verify the lanes that depend on your live data.

<Note>
**Migration is additive.** The migration will only create new agents; your existing assets will not be disrupted. Your pathways, their phone numbers, tools, snippets, and live traffic are never modified. You switch traffic to the new agent only when you decide to by promoting your agent to production.
</Note>
...
Until then you have both dashboards. If your account was created after 18 September 2026 you are on V2 only.

What an automatic migration cannot do is check the result. It makes every choice on your behalf and hands you an agent nobody has looked at. Migrating yourself is how you see what came across, confirm the settings are the ones you want, and test the agent before it answers anyone. That is the reason to do this by hand, rather than the date.

Enterprise accounts do not need to do this themselves. Your Bland partner runs the migration with you.

There is no cost to starting early. The migration is additive, your pathway keeps answering throughout, and a finished agent can sit unpromoted for as long as you like. The work is in reviewing what came across, so the earlier you start, the more of that review happens while you still have both to compare.
...
<CardGroup cols={3}>
  <Card title="Migrate with your agent" icon="terminal">
    Use your own agent (like Claude Code, Codex or Grok Bot) with the Bland MCP plugin to rebuild, test, and iterate. Full control over your migration.
  </Card>
  <Card title="Migrate with Bland" icon="server">
    Bland runs the full agent-driven migration described below on our infrastructure, and you watch live status. Expect 30 to 90 minutes for a complex pathway.
  </Card>
  <Card title="Import" icon="bolt">
    A mechanical 1:1 conversion, good for simple pathways you want to start iterating on as agents.
  </Card>
</CardGroup>
