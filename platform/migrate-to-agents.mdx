...
Everything else is rebuilt by hand. Dispositions, triggers, phone number settings, test cases and test configurations do not come across, because they are not part of a pathway or a persona. That is worth knowing before you start, because "migration" sounds like it covers the account.
...
### Moving traffic gradually

You do not have to switch everything at once. On the agent, under **Dispatch** then **Channels**, **Traffic split from v1** lets you pick the pathway you migrated from and send a percentage of its calls to the new agent.

Your phone numbers and API integrations keep pointing at the pathway. Nothing is repointed. The percentage is rolled per call, so a caller who reaches the agent once may reach the pathway on their next call.

Every failure runs the pathway: an error, a slow lookup, or no split configured all fall back to what you had. Setting the slider to 0% or removing the split returns every call to the pathway from the very next one, and that rollback keeps working even if the feature itself is switched off for your organization.
