...
<Note>
This is a **local** stdio server bundled with the CLI. Bland also hosts a **remote** MCP server at `https://api.bland.ai/v1/mcp` with no local install required. See the [MCP documentation](/integrations/mcp/overview) for setup across Claude Code, Cursor, Codex, and other clients.
</Note>
