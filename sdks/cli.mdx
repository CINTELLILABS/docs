...
The `bland mcp` command below is a **local** stdio server bundled with the CLI. Reach for it when you specifically want a local process, or when your agent has no API key yet: the local server exposes `bland_auth_login` and `bland_auth_poll`, so an agent can [onboard itself](/platform/connect-your-agent#try-it-with-a-client) and get its own key and phone number.
</Note>
