...
const karen = await tts.getVoice("Karen");
...
<Warning>
  `speak()` and the `speak` command generate a complete file over `POST /v1/speak`. They are for producing audio, not for measuring streaming latency or building a voice agent. For streaming, time to first byte, and LLM token input, use [Realtime Speech (WebSocket)](/api-v2/post/tts-ws) directly; for a complete utterance with a streamed response, use [Synthesize Speech (HTTP)](/api-v2/post/tts).
</Warning>
...
Hook into Claude Code, Cursor, Claude Desktop, Windsurf, and other MCP-compatible clients. This server exposes the package's TTS and voice tools only.
