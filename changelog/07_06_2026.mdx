...
Bland's memory now syncs to your CRM at the contact level after every call and message — no tool setup or per-call request data required. Connect HubSpot or Salesforce through a single OAuth modal and sync turns on automatically, pushing each contact's identity, rolling conversation summary, memory entities (bookings, orders, tickets), and deals/opportunities straight onto the contact record.
...
Outbound SIP calls can now use your own internal identities — DIDs and extensions — instead of being forced onto a Bland phone number, enabling true trunk-to-trunk hand-off where Bland passes the call to your system and you take it from there.
...
- Fixes to the new Twilio trunk so the outbound INVITE carries the correct FROM and TO instead of duplicating the source number
