...
- Fix where pathway routing after webhook response code wouldn't respect created rule
- Fix where users were not able to properly delete unused voices at times
- Improvements to warm transfer call failure handling
- Added warm transfer hold music dropdown + preview in the dashboard
- Resolved an edge case where "wait for greeting" and "block interruptions" would not work in the start node
- Analysis presets now auto-save after changes
- Added filters for SMS conversations dashboard
- Fixed scrolling issues in pathway UI under test cases
