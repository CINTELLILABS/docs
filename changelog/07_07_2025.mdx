---
title: 'July 7, 2025'
description: 'Latest updates and improvements to our voice-AI platform'
---

### SIP Outbound Calling (Enterprise)

Bland now supports outbound SIP, allowing you to route outbound calls through your own SIP provider.

- Configure routes using our `/sip` API endpoints
- Works with standard SIP providers and enterprise telephony systems
- Available to Enterprise customers only — contact support to enable access

Use our API to attach, verify, update, and detach SIP routes. Full details—including schema definitions, security requirements, and usage patterns—are available in our [SIP setup guide](/enterprise-features/SIP-integration).

---

### Global Navigation Redesign

We’ve begun rolling out a visual and structural update to Bland's global UI.

- New sidebar, top bar, and bottom bar components
- Updated buttons, tabs, and layout patterns with our refreshed designs
- Not final! More polish, hierarchy work, and tokenization is coming

This is a **foundational update**—setting the stage for a more modern and unified design experience across the platform.


<Tabs>
  <Tab title="After - Settings">
<img 
  src="../changelog_assets/07_07_2025/new_design_settings.png" 
  style={{ width: "100%", borderRadius: "0.5rem", marginTop: "1rem", marginBottom: "1rem" }}
  alt="New Navigation Preview"
/>
  </Tab>
  <Tab title="Before - Settings">
    <img 
      src="../changelog_assets/07_07_2025/old_design_settings.png" 
      style={{ width: "100%", borderRadius: "0.5rem", marginTop: "1rem", marginBottom: "1rem" }}
      alt="Old Navigation Preview"
    />
  </Tab>
  <Tab title="After - Analytics">
    <img 
      src="../changelog_assets/07_07_2025/new_design_analytics.jpeg" 
      style={{ width: "100%", borderRadius: "0.5rem", marginTop: "1rem", marginBottom: "1rem" }}
      alt="New Navigation Preview"
    />
  </Tab>
  <Tab title="Before - Analytics">
    <img 
      src="../changelog_assets/07_07_2025/old_design_analytics.png" 
      style={{ width: "100%", borderRadius: "0.5rem", marginTop: "1rem", marginBottom: "1rem" }}
      alt="Old Navigation Preview"
    />
  </Tab>
</Tabs>
---

### Improvements

- **BLA-3318**: Fixes issue with phone number drift in OTP auth for international numbers
- **BLA-3065**: Adds call status to the CSV export from the calls page
- **BLA-3148**: Fixes voice name mismatch between FE and actual call voice
- **BLA-3175**: Enforces personal BYOT geo-permissions for blocking inbound/outbound calls from banned countries
- **BLA-3274**: Fixes test case error where status was not being updated on failed tests
- **BLA-3192**: Adds "..." silence markers to chat history to support better prompting UX
- **BLA-3281**: Fixes to improve OTP submission success rate on login
