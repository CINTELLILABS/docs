...
- Custom tools on nodes now speak their configured filler line while the tool runs, instead of leaving the caller in silence
...
- Fixed calls restarting at a pathway's start node after an unanswered warm transfer or mid-call reconnection
- Triggered guardrails no longer disappear from past call logs after promoting a new pathway version
...
- Eval agent names now identify each column in eval reports

**Speech Studio**
- Meet the Director, an AI dialogue author for multi-speaker TTS scripts, with performance tags like `[laughs]` and per-speaker voice assignment
- Fine-tuned custom voices no longer fail on longer generations through the `/speak` endpoint

**Calls**
- Dialogue spoken after a webhook no longer fuses words in the transcript or drops audio mid-sentence
- Agent responses that mention AI company names are no longer silently rewritten
...
- [Enterprise] SSO providers can now be edited in place, so you can rotate an expiring client secret or certificate without recreating the provider. Provider creation for Azure AD, Google, Okta, and Auth0 also works again, with clearer errors
- Billing actions are now restricted to organization admins and owners
...
**Automations**
- The Send Call action can attach custom metadata to calls, included in the post-call webhook for every status (including no-answer) so you can correlate webhooks to your records

**Messaging**
- SMS log exports now include a metadata column
- SMS status indicators now show "Sending" and distinguish "Sent" from carrier-confirmed "Delivered"

**Agent Testing**
- Run a scenario N times to see a success rate, and runs that fail before the first turn get a "Failed to start" badge

**Releases**
- The [Releases page](https://app.bland.ai/dashboard/settings/releases) now works on regional domains (EU, CA, AU, and Asia)

**Analytics**
- Call durations now render correctly across board panels, and charts show the correct dates in all timezones
