...
<Note>
  Performance tags can underperform on an instant clone. When a turn uses one of your instant clones, the studio shows a note beside the Effects panel explaining two fixes: edit the clone's transcript to label real effects in the recording (open **Voice samples** on the clone and mark a real sigh as `[sighs]`; editing is only available on clones that carry a transcript), or train a [studio clone](/tts/voices#studio-clones), which is more accurate with tags.
</Note>
