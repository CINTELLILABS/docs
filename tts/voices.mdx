...
The dashboard offers two clone types. Start a clone at
[Voices → Studio](https://app.bland.ai/dashboard/voices?tab=studio) and pick one from the
side-by-side chooser:

- **Instant clone**: built from one clean sample of about ten seconds, usable immediately.
- **Studio clone**: trained from 10 minutes to 2 hours of audio (30 minutes recommended).
  Higher fidelity, and more accurate with performance tags. Studio clones carry a **Pro** tag
  in your library.

A cloned voice of either type is private to your organization.

### Instant clones

Quality of the sample sets the ceiling on quality of the voice, so it is worth trimming
properly:
...
Create an instant clone from the dashboard, or programmatically with
[Clone Voice](/api-v1/post/clone).
...
A studio clone runs a training job on the audio you upload. Before you can start the run,
check the consent box confirming you have the speaker's consent (or own the voice) and accept
the voice training terms. Training does not start without it.

While the run is in flight:

- The builder shows live training progress, including the loss curve and a tail of the
  training log.
- You can cancel the run from the progress view. Canceling stops the run; it does not produce
  a voice.
- The run stays visible under **My voices** with a **Training** badge. Select it to reopen
  the progress view, or select an unsubmitted draft to resume editing it.

See [Limits](/speech/limits) for how many clones your organization can have active.

## Reviewing a voice's generations

Under **My voices**, open a voice's row menu and select **Generations** to see the synthesis
history for that voice. Each entry plays back in place and can be downloaded.
