...
Retired voices no longer appear in [List Voices](/api-v1/get/voices) or in Studio, and new
saves or publications that reference them are rejected. Existing agents, calls, and pathways
that still hold a retired voice ID keep running: synthesis for those requests uses
[Karen](#built-in-voices) so calls start speaking immediately instead of waiting on the
retired provider.

Update the stored voice to a currently-supported one when you want a voice other than Karen.
