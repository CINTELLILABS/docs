...
### Editing an instant clone's transcript

`BTTS_V3` instant clones condition synthesis on a transcript of the source recording. If that
transcript picked up a misheard word during cloning, correcting it can improve later output
without retraining. Read the current transcript with
[Get Clone Info](/api-v1/get/voices-id-clone), replay the source audio with
[Get Clone Audio](/api-v1/get/voices-id-clone-audio), and save a fix with
[Update Clone Transcript](/api-v1/patch/voices-id-clone-transcript). The next synthesis picks
